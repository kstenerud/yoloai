...
func TestApplyPatch_GitTarget(t *testing.T) {
	...
	require.NoError(t, workspace.ApplyPatch(patch, targetDir, true))
...
func TestCheckPatch_Conflict(t *testing.T) {
	...
	err = workspace.CheckPatch(patch, targetDir, true)
	assert.Error(t, err)
}
...
func TestHasUncommittedChanges_Clean(t *testing.T) {
	...
	rt := getTestRuntime(t)
	has, err := HasUncommittedChanges(context.Background(), rt, "test-wip-clean")
	require.NoError(t, err)
	assert.False(t, has)
}
...
func TestHasUncommittedChanges_Modified(t *testing.T) {
	...
	rt := getTestRuntime(t)
	has, err := HasUncommittedChanges(context.Background(), rt, "test-wip-mod")
	require.NoError(t, err)
	assert.True(t, has)
}
...
func TestHasUncommittedChanges_NewFile(t *testing.T) {
	...
	rt := getTestRuntime(t)
	has, err := HasUncommittedChanges(context.Background(), rt, "test-wip-new")
	require.NoError(t, err)
	assert.True(t, has)
}
...
func TestHasUncommittedChanges_OnlyCommits(t *testing.T) {
	...
	rt := getTestRuntime(t)
	has, err := HasUncommittedChanges(context.Background(), rt, "test-wip-committed")
	require.NoError(t, err)
	assert.False(t, has)
}
...
func TestGenerateFormatPatch_Single(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-fp-one", nil)
	require.NoError(t, err)
...
func TestGenerateFormatPatch_Multiple(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-fp-multi", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestGenerateFormatPatch_NoCommits(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-fp-empty", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestGenerateFormatPatch_PathFilter(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-fp-filter", []string{"a.txt"})
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestGenerateWIPDiff_NoChanges(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patch, stat, err := GenerateWIPDiff(context.Background(), rt, "test-wip-diff-none", nil)
	require.NoError(t, err)
	assert.Empty(t, patch)
	assert.Empty(t, stat)
}
...
func TestGenerateWIPDiff_Modified(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patch, stat, err := GenerateWIPDiff(context.Background(), rt, "test-wip-diff-mod", nil)
	require.NoError(t, err)
	assert.NotEmpty(t, patch)
...
func TestGenerateWIPDiff_PathFilter(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patch, stat, err := GenerateWIPDiff(context.Background(), rt, "test-wip-diff-filter", []string{"file.txt"})
	require.NoError(t, err)
	assert.NotEmpty(t, patch)
	assert.Contains(t, stat, "file.txt")
	assert.NotContains(t, stat, "other.txt")
...
func TestApplyFormatPatch_Single(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-am-one", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestApplyFormatPatch_Multiple(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-am-multi", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
	...
	cmd := workspace.NewGitCmd(targetDir, "rev-list", "--count", "HEAD")
	out, err := cmd.Output()
	require.NoError(t, err)
...
func TestApplyFormatPatch_Conflict(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-am-conflict", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestApplyFormatPatch_PreservesAuthorship(t *testing.T) {
	...
	rt := getTestRuntime(t)
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-am-author", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestAdvanceBaseline_RWNoop(t *testing.T) {
	...
	rt := getTestRuntime(t)
	assert.NoError(t, AdvanceBaseline(context.Background(), rt, "test-advance-rw"))
}
...
func TestContiguousPrefixEnd_AllApplied(t *testing.T) {
	...
	assert.Equal(t, 2, workspace.ContiguousPrefixEnd(commits, applied))
}
...
func TestContiguousPrefixEnd_PartialPrefix(t *testing.T) {
	...
	assert.Equal(t, 1, workspace.ContiguousPrefixEnd(commits, applied))
}
...
func TestContiguousPrefixEnd_Gap(t *testing.T) {
	...
	assert.Equal(t, 0, workspace.ContiguousPrefixEnd(commits, applied))
}
...
func TestGenerateFormatPatchForRefs_Single(t *testing.T) {
	...
	patchDir, files, err := GenerateFormatPatchForRefs(context.Background(), rt, "test-fpref-single", []string{commits[1].SHA}, nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestGenerateFormatPatchForRefs_Multiple(t *testing.T) {
	...
	patchDir, files, err := GenerateFormatPatchForRefs(context.Background(), rt, "test-fpref-multi", []string{commits[0].SHA, commits[2].SHA}, nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestSelectiveApplyFlow(t *testing.T) {
	...
	patchDir, files, err := GenerateFormatPatchForRefs(context.Background(), rt, name, shas, nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
	...
	prefixEnd := workspace.ContiguousPrefixEnd(commits, appliedSet)
...
func TestApplyFlow_CommitsOnly(t *testing.T) {
	...
	hasWIP, err := HasUncommittedChanges(context.Background(), rt, "test-flow-commits")
	require.NoError(t, err)
	assert.False(t, hasWIP)

	// Generate and apply
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-flow-commits", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
...
func TestApplyFlow_CommitsAndWIP(t *testing.T) {
	...
	hasWIP, err := HasUncommittedChanges(context.Background(), rt, "test-flow-both")
	require.NoError(t, err)
	assert.True(t, hasWIP)

	// Apply commits first
	patchDir, files, err := GenerateFormatPatch(context.Background(), rt, "test-flow-both", nil)
	require.NoError(t, err)
	defer os.RemoveAll(patchDir) //nolint:errcheck
	...
	// Then apply WIP
	wipPatch, _, err := GenerateWIPDiff(context.Background(), rt, "test-flow-both", nil)
	require.NoError(t, err)
...
func TestApplyFlow_WIPOnly(t *testing.T) {
	...
	hasWIP, err := HasUncommittedChanges(context.Background(), rt, "test-flow-wip")
	require.NoError(t, err)
	assert.True(t, hasWIP)
