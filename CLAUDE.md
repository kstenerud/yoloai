...
**Doc conventions.** Every directory's `README.md` is its index. Filenames are lowercase kebab-case and name the subject, not its status. Three content-retirement patterns: **item-queues** keep active items in `<topic>-unresolved.md` and drain each item to one of three co-located sinks — `<topic>-resolved.md` (done: answered/fixed/applied), `<topic>-deferred.md` (parked "not now"; carries a **`Trigger:`** revival condition and can flow back to unresolved), or `<topic>-abandoned.md` (permanently "won't do"; carries a **`Why:`**) — for critiques, questions, and findings; **append-only logs** (`decisions/`) grow and age-split; **file-documents** (plans, specs, research spikes) move whole to `archive/` when complete.

## Architecture (from design docs)

- Go binary, no runtime deps — just the binary and Docker (or Tart for macOS VMs, or Seatbelt for lightweight macOS sandboxing). One narrow exception: in-place `reset` (resetting a sandbox while the agent keeps running, container backends only) shells out to host `rsync` for a differential workdir resync — see `rsyncDir` in `internal/orchestrator/lifecycle/reset.go` for why a wipe-then-copy can't be used there. It is not needed for normal create/run/restart. `rsync` is **required** tooling (D112): the lifecycle in-place-reset tests FAIL if it's absent — install it (no carve-out; it installs everywhere including CI).
- Pluggable runtime backend via `runtime.Backend` interface in `runtime/`. Three backends: Docker (`runtime/docker/`), Tart (`runtime/tart/`), and Seatbelt (`runtime/seatbelt/`). CLI dispatches via `newRuntime()` in `internal/cli/helpers.go`. No backend-specific types leak outside their packages.
...
**Before considering any code change complete, run `make check`.** This runs gofmt verification, golangci-lint, go mod tidy check, the tagged-test typecheck (`vet-tagged`), the macOS cross-compile typecheck (`crosscheck`), all Go tests, and the Python test/typecheck targets. All must pass before committing. If `make check` fails, fix the issues before proceeding. Subagents implementing code changes must include `make check` as a final step.

`crosscheck` (`GOOS=darwin GOARCH=arm64 go vet ./...`) cross-compiles + typechecks the whole tree — the `cmd/yoloai` binary and every `_test.go` — for macOS, the non-host platform. The host build only covers linux, so a darwin-only break (a backend whose Go deps build only on linux like `runtime/containerd`, an unconditional import of one, a `//go:build` typo) otherwise surfaced only when someone ran `make releasetest` on a Mac. Windows isn't a target (the project runs under WSL = linux; `docker`/`tart` are `//go:build !windows`), so only darwin is cross-checked.

The Python portion (`make python-test` + `make python-typecheck`) covers the typed pure-function surface in `runtime/monitor/setup_helpers.py` and its tests under `runtime/monitor/tests/`, plus the smoke harness. mypy/pytest run from a [uv](https://docs.astral.sh/uv/)-managed `.venv` pinned to exact versions by `runtime/monitor/tests/requirements-dev.lock` — this decouples the checks from whatever ambient `python3 -m mypy` is installed (an out-of-spec ambient mypy was producing phantom `--strict` errors). The targets self-provision the venv on demand (no manual step). `uv` is **required** (D112): the Python surface is app code, so `make check` FAILS without `uv` — install it (no carve-out; `uv`/`python3` install everywhere including CI). `make setup-dev-python` is an optional explicit provision (used by CI, fails loudly if uv is missing). After editing `requirements-dev.txt`, regenerate the lock with `uv pip compile --generate-hashes`.
...
- **Critique cycle:** Write a critique in `docs/contributors/design/critiques-unresolved.md`, apply corrections to design docs and research files in `docs/contributors/design/research/`, then drain the item to one of the three sinks: `critiques-resolved.md` once applied, `critiques-deferred.md` if parked "not now" (must add a `Trigger:` line stating what revives it), or `critiques-abandoned.md` if dropped (must add a `Why:` line). A deferred item flows back to the unresolved file when its trigger fires. Findings discovered mid-work follow the same flow via `findings-unresolved.md` → `resolved-/deferred-/findings-abandoned.md`; open questions likewise via `questions-unresolved.md`.
