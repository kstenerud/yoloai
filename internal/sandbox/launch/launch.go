...
func LaunchContainer(ctx context.Context, d state.Deps, st *state.State) error {
	...
	secretsDir, err := provision.CreateSecretsDir(st.Agent, envVars, st.Isolation, st.CredOverrides)
	if err != nil {
...
func applyOverlayAndCaps(st *state.State, caps runtime.BackendCaps, instanceCfg *runtime.InstanceConfig, runtimeName runtime.BackendName) error {
