...
func mutateBaseline(ctx context.Context, layout config.Layout, rt runtime.Backend, name, dirHostPath, expectedCurrentSHA string, resolve func(workDir string) (string, error)) (*BaselineChange, error) {
	...
	if err := checkBaselineMode(dir.Mode); err != nil {
		return nil, err
	}

	sha, err := commitBaseline(sandboxDir, meta, dirHostPath, &expectedCurrentSHA, resolve)
	if err != nil {
		return nil, err
	}
	...
	g := git.NewSandbox(layout, rt, name)
	subject := ""
	if out, subjErr := g.Run(ctx, workDir, "log", "--format=%s", "-1", sha); subjErr == nil {
...
func commitBaseline(sandboxDir string, meta *store.Environment, dirHostPath string, expected *string, resolve func(workDir string) (string, error)) (string, error) {
	dir := meta.Dir(dirHostPath)
	if dir == nil {
		return "", fmt.Errorf("directory %q not found in sandbox %q", dirHostPath, sandboxDir)
	}
	if expected != nil && dir.BaselineSHA != *expected {
		return "", &BaselineConflictError{Expected: *expected, Actual: dir.BaselineSHA}
	}
	workDir := store.WorkDir(sandboxDir, dir.HostPath)
...
func BaselineLog(ctx context.Context, layout config.Layout, rt runtime.Backend, name string, dirHostPath string) ([]BaselineLogEntry, error) {
	...
	inceptionSHA := dir.InceptionSHA
	if inceptionSHA == "" {
		if out, gitErr := g.Run(ctx, workDir,
	...
	if inceptionSHA == "" {
		out, logErr := g.Run(ctx, workDir, "log", "--format=%H %s")
		if logErr != nil {
	...
	out, err := g.Run(ctx, workDir, "log", "--format=%H %s", inceptionSHA+"..HEAD")
	if err != nil {
		return nil, fmt.Errorf("git log: %w", err)
	}
	entries := parseBaselineLog(out, baselineSHA)

	inceptionLine, err := g.Run(ctx, workDir, "log", "--format=%H %s", "-1", inceptionSHA)
	if err != nil {
