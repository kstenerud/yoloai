...
// traversalPatch names a path that escapes the target directory. A :copy work
// dir's patch is generated by a git diff running INSIDE the sandbox, so it is
// attacker-influenced; this shape is the DF70 host-file-write primitive.
const traversalPatch = `diff --git a/../escape.txt b/../escape.txt
...
// TestApplyPatch_NonGitDir_RejectsTraversal is the DF70 regression guard: the
// non-repo apply path must refuse a patch that writes outside the target, and
// nothing may land on the host. The old `--unsafe-paths --directory=` route
// disabled git's path-safety check and silently wrote the escaping file.
func TestApplyPatch_NonGitDir_RejectsTraversal(t *testing.T) {
	parent := t.TempDir()
	targetDir := filepath.Join(parent, "target")
	require.NoError(t, os.Mkdir(targetDir, 0o750))
	writeTestFile(t, targetDir, "file.txt", "old content\n")

	err := NewTestHostWithEnv(testEnv()).ApplyPatch(ctx, []byte(traversalPatch), targetDir, false)
	assert.Error(t, err, "traversal patch must be rejected")

	_, statErr := os.Stat(filepath.Join(parent, "escape.txt"))
	assert.True(t, os.IsNotExist(statErr), "escaping file must not be written outside the target")
}

// TestCheckPatch_NonGitDir_RejectsTraversal is the DF70 guard for the dry-run
// path — --check must measure the same working area as the real apply.
func TestCheckPatch_NonGitDir_RejectsTraversal(t *testing.T) {
	parent := t.TempDir()
	targetDir := filepath.Join(parent, "target")
	require.NoError(t, os.Mkdir(targetDir, 0o750))
	writeTestFile(t, targetDir, "file.txt", "old content\n")

	err := NewTestHostWithEnv(testEnv()).CheckPatch(ctx, []byte(traversalPatch), targetDir, false)
	assert.Error(t, err, "traversal patch must be rejected at --check")
}

// TestApplyPatch_NonGitDir_AmbientRepoDoesNotWiden guards the subtler DF70
// case: a :copy target nested under an unrelated git repo. git must not adopt
// that ambient repo as the working area (which would let `../…` escape the
// target while still landing on the host); the throwaway git-dir blocks the
// upward search, while a legitimate in-target patch still applies.
func TestApplyPatch_NonGitDir_AmbientRepoDoesNotWiden(t *testing.T) {
	outer := t.TempDir()
	initGitRepo(t, outer) // ambient repo ABOVE the target
	targetDir := filepath.Join(outer, "target")
	require.NoError(t, os.Mkdir(targetDir, 0o750))
	writeTestFile(t, targetDir, "file.txt", "old content\n")

	host := NewTestHostWithEnv(testEnv())

	err := host.ApplyPatch(ctx, []byte(traversalPatch), targetDir, false)
	assert.Error(t, err, "traversal must be rejected even under an ambient repo")
	_, statErr := os.Stat(filepath.Join(outer, "escape.txt"))
	assert.True(t, os.IsNotExist(statErr), "escaping file must not land in the ambient repo")

	// A legitimate in-target patch still applies despite the ambient repo.
	repoDir := t.TempDir()
	initGitRepo(t, repoDir)
	good := generatePatch(t, repoDir, "file.txt", "old content\n", "new content\n")
	require.NoError(t, host.ApplyPatch(ctx, good, targetDir, false))
	content, readErr := os.ReadFile(filepath.Join(targetDir, "file.txt")) //nolint:gosec // G304: test file path
