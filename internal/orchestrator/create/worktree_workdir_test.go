...
// DF121: :rw and :ro track no baseline. `yoloai baseline` says so to the user's
// face ("baseline is not tracked for :rw directories") and diff substitutes the
// literal HEAD for a live mount, so recording the source's HEAD produced a value
// nothing read, that `sandbox info` printed anyway, and that went stale on the
// next commit. Asserted through setupAuxDir, the real path for these modes.
func TestSetupAuxDir_RWTracksNoBaseline(t *testing.T) {
	assertAuxDirHasNoBaseline(t, "rw")
}

func TestSetupAuxDir_RODirTracksNoBaseline(t *testing.T) {
	assertAuxDirHasNoBaseline(t, "ro")
}

func assertAuxDirHasNoBaseline(t *testing.T, mode string) {
	t.Helper()
	dir := t.TempDir()
	testutil.InitGitRepo(t, dir)
	writeTestFile(t, dir, "app.js", "v1")
	...
	dirEnv, err := setupAuxDir(context.Background(),
		git.NewTestHostWithEnv(testutil.GitEnv()),
		filepath.Join(t.TempDir(), "sandbox"),
		&mockDockerRuntime{},
		&DirSpec{Path: dir, Mode: DirMode(mode)})
	require.NoError(t, err)
	assert.Empty(t, dirEnv.BaselineSHA, "a live mount has no work copy to baseline, and the rest of the code already agrees")
}
