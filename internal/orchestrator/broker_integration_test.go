...
// ABOUTME: keeps the API key host-side, points the agent at the injector, and the
// ABOUTME: container reaches the injector (gateway) which swaps in the real key.
...
// real Docker: the real key stays host-side, the container env is rewritten to
// point at the injector with a placeholder, and a request from inside the
// container flows container -> bridge gateway -> injector -> mock upstream with
// the real key swapped in host-side. The injector is reaped on destroy.
//
...
func TestIntegration_CredentialBroker(t *testing.T) {
	...
	// Host-side mock "upstream": the injector forwards here (loopback, host-side)
	// instead of real api.anthropic.com. The container never talks to it directly.
	...
	// Make the keepalive "test" agent brokerable, pointing its upstream at the
	// mock. The bash keepalive keeps the container up to exec into, decoupling the
	// test from driving a real agent's wire protocol. Restored after the test.
	...
	_, err := createSandbox(ctx, mgr, orchestrator.CreateOptions{
		...
		Agent:   "test",
		// The agent process has the brokered env; dump it to a file so exec (a
		// fresh process) can read it. Keepalive sleep keeps the agent alive across
		// the assertions.
	...
	// Brokering is auto-on (default): docker is InjectorReachable, the agent is
	// brokerable, the key is present, and networking is open.
	const realKey = "sk-test-REAL-broker-key-xyz"
	...
	// 1. The real key never enters the container.
	_, hasKey := env["TEST_BROKER_KEY"]
	assert.False(t, hasKey, "real key must not enter the container env")

	// 2. The agent is pointed at the injector with the placeholder credential.
	...
	//    bridge gateway; the injector swaps the placeholder for the real key and
	//    forwards to the mock, streaming the response back. Explicit argv avoids
	//    any shell quoting of the header/url.
	...
	assert.Equal(t, realKey, gotKey, "the real key is injected host-side (seen only at the host mock)")
	assert.Empty(t, gotAuth, "the placeholder Authorization is stripped before the upstream")
