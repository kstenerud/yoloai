...
func TestGetAgent_Claude(t *testing.T) {
	...
	require.Len(t, def.AuthFiles, 1)
	assert.Equal(t, "~/.claude/.credentials.json", def.AuthFiles[0].HostPath)
	assert.Equal(t, ".credentials.json", def.AuthFiles[0].TargetPath)
