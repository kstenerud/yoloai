...
func runMCPServe(cmd *cobra.Command, _ []string) error {
	backend, warn := yoloai.SelectContainerBackend(cmd.Context(), cliutil.ResolveContainerBackendConfig(), cliutil.Layout().Env().EnvForDaemonDiscovery())
	if warn != "" {
...
func runMCPProxy(cmd *cobra.Command, args []string) error {
	...
	if rawWorkdir != "" {
		parsed, err := cliutil.ParseDirArg(rawWorkdir, homeDir, interpEnv)
		if err != nil {
	...
	// be :copy (diff/apply is workdir-only).
	var auxDirSpecs []yoloai.DirSpec
	for _, rawDir := range rawDirs {
		parsed, err := cliutil.ParseAuxDirArg(rawDir, homeDir, interpEnv)
		if err != nil {
