...
| Model aliases                 | `sonnet` → `claude-sonnet-4-latest`, `opus` → `claude-opus-4-latest`, `haiku` → `claude-haiku-4-latest` | `pro` → `gemini-2.5-pro`, `flash` → `gemini-2.5-flash` | (none) | (none yet) | (none) |
...
| Proxy support                 | Yes (npm install only, not native binary)                 | TBD                                             | N/A                                             | TBD (research needed)                          | N/A                                            |
| Default network allowlist     | `api.anthropic.com`, `statsig.anthropic.com`, `sentry.io` | `generativelanguage.googleapis.com`             | (none)                                          | `api.openai.com` (minimum; additional TBD)     | Union of all agents' allowlists                |
