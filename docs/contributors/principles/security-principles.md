...
Established in D22 (`../working-notes.md`). Primary-source backing: `../research/principles/security-principles-research.md`. Concrete implementation: `docs/contributors/design/security.md` and `docs/contributors/design/network-isolation.md`.
...
- **`docs/contributors/design/network-isolation.md` §Threat Model** states the boundary explicitly: "Sandbox escape via kernel/runtime exploit. This is gVisor's and Kata's domain. Network isolation cannot help if the agent breaks out of the sandbox entirely." Acknowledging the limit is the principle in action.
...
- **Dangerous-directory refusal** (`docs/contributors/design/security.md`): refuses to mount `$HOME`, `/`, `/etc`, `/usr`, etc. The agent can't write there because we never mounted there.
...
- **`/run/secrets/<key_name>`** scheme documented in `docs/contributors/design/security.md` §Credential Management.
...
- **Podman behaviour claims** verified against source: commit `77f9dab` (2026-03-15, "Verify Podman research claims against Docker/Podman/Buildah source"). Where Podman differs from Docker on rootless UID mapping, HOME handling, tmux exec — those are documented in `docs/contributors/design/research/podman.md` from observed behaviour, not inferred from docs.
- **gVisor permission claims** verified by running yoloAI under gVisor and observing the bind-mount permission failures. The fix (relaxed 0777/0666 for container-writable paths, documented in `docs/contributors/design/security.md`) was empirical, not assumed.
...
- **Backend idiosyncrasies catalog** (`docs/contributors/backend-idiosyncrasies.md`): every entry is symptom + explanation + fix + code pointer. No "the vendor docs say…" hand-waving.
...
Originally established alongside D11 and `docs/contributors/design/security.md`.
...
Threshold: every isolation-undermining feature is a discrete opt-in with a documented rationale and a documented residual risk. The user types the feature; the tool doesn't infer it. Combined opt-ins (e.g., `:overlay` + `--network-isolated`) require multiple capabilities, each named in `docs/contributors/design/security.md`.

### Worked examples

- **`cap_add` in profile recipes** (`docs/contributors/design/config.md`): the user can grant additional capabilities for power-user features (e.g., Tailscale needs `NET_ADMIN`, GPU passthrough needs `--device`). Documented in `docs/contributors/design/security.md` §Security Considerations as a "no guardrails — a misconfigured recipe could undermine container isolation" path.
...
- **`directories:` config** for direct bind-mounts (e.g. a host path carrying `${SSH_AUTH_SOCK}`): explicit credential surface granted by the user, documented in `docs/contributors/design/security.md` with the recommended way to forward SSH (via agent socket, not by mounting `~/.ssh/`). This was the `mounts:` key until [D142](../decisions/working-notes.md) retired it; `directories:` is its strict superset and additionally carries the dangerous-path, overlap and dirty-repo guards `mounts:` had none of.
...
`docs/contributors/design/security.md`; `general-principles.md §6`. Full citations: `../research/principles/security-principles-research.md §10`.
...
- **`agent_files`' two forms drifted, and the divergence was the vulnerability (DF201).** The agent's `AgentFilesExclude` denylist — `.credentials.json`, `oauth_creds.json`, `auth.json`, `sessions/` — was applied by the string form and ignored by the list form, so `agent_files: ["~/.claude"]` copied live credentials into a sandbox that `agent_files: "~"` would have stripped. Neither guard was individually wrong; one was simply absent, on the path nothing pointed at. `docs/GUIDE.md` documented the exclusions without qualifying them to a form, so the docs described the safe half and users inherited the unsafe one. **The denylist now holds for both forms, and that is a claim rather than an intention: `TestArch_AgentFilesNeverCopiesExcludedCredentials`.** Note what makes this the canonical shape of the principle — an audit *reading* either implementation alone finds it correct.
- **Ambient environment reading is confined to one seam ([D144](../decisions/working-notes.md#d144--ambient-environment-read-once-at-the-cli-name-freely-in-an-agent-definition-never-grant-invisibly)).** `os.Getenv`, `os.Environ`, `os.LookupEnv`, `os.ExpandEnv`, `syscall.Getenv`, and `syscall.Environ` are forbidden repo-wide by `forbidigo`, with one path exemption (`cliutil/layout.go`'s licensed `os.Environ()`); everything below reads the threaded snapshot (`config.Layout`) as data. That only proves the *call* is absent — a linter cannot check that a variable which is live in the process but missing from the threaded snapshot never reaches the sandbox anyway. **That behaviour is the claim, and it is pinned: `TestArch_LibraryNeverReadsAmbientEnv`.** D144's second line is the sibling half of the same decision: an agent definition (shipped or file-defined) may name any env var it wants — that is deliberate, not a violation — but the keys actually injected are disclosed at launch, naming the declaring agent and whether it is user-defined ([DF202](../design/findings-resolved.md)), so the one thing selection may not do is grant invisibly.
...
`development-principles.md §4` (parse, don't validate — the convention this principle protects); `docs/contributors/design/findings-deferred.md` DF15 (canonical live instance). Decisions D58/D59 (the pass that revives DF15).
