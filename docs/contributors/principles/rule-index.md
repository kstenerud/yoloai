...
| DEV §12 | Resolve ambient state (env, `$HOME`, cwd, terminal, identity) once at the outermost edge; library takes explicit args (pure accessor downstream — panic if read pre-edge, never a lazy fallback). Binds **outward** — every subprocess gets an explicit `cmd.Env`, never inherited `os.Environ()` (raw `exec.Command*` banned, funneled through the env-building helper) — and binds **tests identically** (no pass; a test's only edge is its isolation helper). | Calling `os.Getenv` / `os.UserHomeDir` / `os.Getwd` below the CLI, a "just for tests" fallback, or launching a subprocess with the inherited env (child reads an ambient `$HOME`/`TART_HOME`/`DOCKER_HOST` you never set). |
...
| GEN §3   | Check whether git/Docker/unix/the runtime already solve it; build the glue, not the primitives. And read the prior art *before* opening a hardware-verification round, not beside it — a convergence away from your design is a constraint, not a note (D136). | About to hand-build what git/docker/unix already does; about to spend days of hardware time on a mechanism the field abandoned. |
