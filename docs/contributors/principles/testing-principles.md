...
- **Container/process *lifecycle* is e2e-only — unit tests structurally cannot see it.** A unit test with a fake `runtime.Backend` can prove the *orchestration logic* (which calls happen, in what order — e.g. `Create → Start → Ready → Launch`), but it cannot see how the container actually behaves: `docker exec` connection lifetime, init-ordering races, a background monitor's real timing, or a status file's write/read windows. The public-layering carve work (2026-06) is the cautionary tale — the orchestration unit tests were green while a live-docker smoke surfaced three real bugs the fakes could never catch: the launched session-runner died when the CLI exited (attached-exec lifetime) **and** when launched mid-entrypoint-root-setup (a readiness race) — [DF44](../design/findings-unresolved.md); the status-monitor exited on the agent's first turn instead of persisting — DF46; and a stale `idle` left a `Wait` window open on restart. **Rule of thumb: any change to the launch / session / completion *orchestration* needs a real-container smoke before it's trusted, not just the unit test that proves the call sequence.** (Smoke recipe: rebuild the binary; image rebuilds only when an embedded script changes; `yoloai new … -p "<prompt>"`; introspect with `docker exec`.)
