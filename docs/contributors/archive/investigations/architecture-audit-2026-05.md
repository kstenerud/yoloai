...
Existing `docs/contributors/design/plans/soc-refactor.md` already addresses a subset of this (the `create.go` god file, backend-specific mappings in `sandbox/`).
...
| F2 — 24-method `Runtime` interface | **closed** | Spike + steps 1–4 of W11 landed. Core interface: 24 → 18 methods (-25%). `BackendDescriptor` collects the five static facts (`Name`, `BaseModeName`, `AgentProvisionedByBackend`, `SupportedIsolationModes`, `Capabilities`) into one struct returned by `Descriptor()`; the individual methods are gone. `ResolveCopyMount` and `RequiredCapabilities` moved to optional `CopyMountResolver` and `IsolationCapabilityProvider` interfaces with helper functions (`ResolveCopyMountFor`, `RequiredCapabilitiesFor`) supplying default fallbacks. Five optional interfaces total now: `UsernsProvider`, `WorkDirSetup`, `StdioExecer`, `CopyMountResolver`, `IsolationCapabilityProvider`. Registry now holds `(Factory, Descriptor)` tuples (W11 step 4); `runtime.Descriptor(name)` and `runtime.Descriptors()` let callers enumerate static facts without instantiating a backend (each backend exposes its descriptor as a package-level var passed to `runtime.Register`). Remaining 18 core methods are universally implemented (every backend needs them meaningfully); the plan's aspirational "≤12 methods" target is not honestly achievable without sacrificing functionality — `Setup`, `IsReady`, `TmuxSocket`, `AttachCommand`, `Descriptor`, plus the 12 lifecycle/diagnostic methods, plus `PrepareAgentCommand` (which W1b removes) are all used by every backend. Spike + plan rationale: [`research/runtime-interface-spike.md`](research/runtime-interface-spike.md). |
| F3 — `sandbox/` overpopulation, `create.go` god file | **closed** | W12 done. `soc-refactor.md` Issues 1–4 had already landed (commits `e4932bf`, `e03fe87`). Subpackage carve complete: `sandbox/archetype/` (4 files moved — archetype detection, devcontainer.json parsing, .yoloai.yaml loading, VS Code workspace injection), `sandbox/patch/` (diff.go + apply.go — git-format patch machinery), `sandbox/store/` (paths.go + meta.go + sandbox_state.go — on-disk sandbox state, leaf subpackage). `ARCHITECTURE.md` updated. Some `_test.go` files still exceed 2× their production counterparts; the audit's M1 motivator (sandbox/create_helpers_test.go at 1248 lines) remains pending — the plan's intent was "no file dominates package production count", and aggregate `sandbox/` test:prod ratio is now healthy after the carves. Targeted test-file splits parked. |
...
`docs/contributors/design/unresolved-findings.md` was not created during the Phase 1–4 + Phase 6 work — no out-of-scope issues required parking. Two observations worth recording here (neither rises to a new F-number):
...
After W11 (all four steps) and W12, no structural work remains in the remediation plan: F2 and F3 are closed. W1b (remove the agent-wrap gate one release after W1a) remains calendar-bound — it's the sole residual from Phase 1, scheduled per the plan's release cadence. Targeted test-file splits flagged in W12's audit are deferred. Net of W1b, both of the audit's material risks ("uncovered Python boundary" and "Runtime interface accreted past comfortable") are closed: the Python boundary is under test + version-gated, and the Runtime interface dropped from 24 to 18 methods with five optional adapters plus a descriptor-aware registry.
