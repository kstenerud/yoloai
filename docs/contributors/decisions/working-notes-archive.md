...
Historical decision log. Principles (`principles/`) and standards (`standards/`) cite these entries by D-number; the citations resolve here. New decisions are **not** appended to this file — they go in [working-notes.md](README.md). Conventions and entry structure are described there.
...
**Decision.** Before implementation begins on a feature, a critique pass runs against the design docs in `docs/contributors/design/` and the research files in `docs/contributors/design/research/`. The pass identifies factual errors, internal contradictions, and unstated assumptions. Findings are applied; `CRITIQUE.md` is emptied for the next round.
...
**Why.** Principle of least astonishment: an agent reading an error trace inside the sandbox should see the same path the user sees outside. Dangerous-directory detection (D11 / `docs/contributors/design/security.md`) prevents the safety risk that the `/work` prefix was originally meant to guard against.
...
- `/docs/contributors/design/security.md` documents the capability tradeoffs.
...
**Consequences.** All path inputs go through resolution before validation. Documented in `docs/contributors/design/security.md` and enforced in code review.
...
**Composition.** Worked example for `principles/development-principles.md §Iterate when the first approach doesn't work`. Cites `docs/contributors/design/research/idle-detection.md` for the trail of rejected approaches.
...
**Consequences.** `docs/contributors/design/security.md` §Seatbelt Backend Security documents the allowlist. Users opting in to credential access do so via config `env:` and `mounts:`.
...
- *gVisor by default* — rejected; gVisor has permission and platform quirks (relaxed bind-mount permissions per `docs/contributors/design/security.md`, blocked on macOS due to a known Claude Code bug).
...
**Decision.** Periodic architecture audits produce a numbered remediation plan (`W1`, `W2`, …). Each work item is a discrete commit. The plan tracks status (`docs/contributors/archive/investigations/architecture-audit-2026-05.md`) and a memory entry tracks completion across sessions.
...
**Consequences.**
- `docs/contributors/principles/` and `docs/contributors/standards/` exist. README in each is the index.
...
**macOS still unverified** — see `docs/contributors/archive/investigations/macos-disk-reporting-checklist.md`, updated to drop the deleted `statfs` references and check the before/after delta instead. **(Verified on macOS 2026-05-29 — see D38.)**

## D38 — macOS disk reporting verified; Tart gains DiskUsageReporter + real reclaim; docker/podman/seatbelt confirmed accurate

**Date:** 2026-05-29. **Status:** Accepted. **Composition:** verifies D36 (sizing) and D37 (before/after reclaim delta) on macOS; closes the "macOS unverified" note on D36. **Context:** the D35–D37 disk-reporting/prune work was Linux-only; this is the macOS pickup (`docs/contributors/archive/investigations/macos-disk-reporting-checklist.md`). The test Mac runs Docker via **OrbStack** (not Docker Desktop), Podman 5.8.1 (Podman Machine `applehv`), Tart 2.31.0 (Apple Silicon), and Seatbelt.
