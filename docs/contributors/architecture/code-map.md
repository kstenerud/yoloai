...
| File | Purpose |
|------|---------|
| `runtime.go` | `Backend` interface — pluggable backend abstraction. Generic types: `MountSpec`, `PortMapping`, `InstanceConfig`, `InstanceInfo`, `ExecResult`, `BackendCaps`, `ResourceLimits`, `PruneItem`, `PruneAction`, `PruneResult`. Optional interfaces: `UsernsProvider`, `WorkDirSetup`. Sentinel errors: `ErrNotFound`, `ErrNotRunning`. The three prune methods (`Prune`, `CachePruner.PruneCache`, `StaleBasePruner.PruneStaleBases`) take **no writer** — what prune produces is a report, so it comes back as `PruneItem`s carrying an `Action` and a `Reason` plus `feedback.Notice`s for what is not about any one resource (D145). `PruneItem.Removable()` is the predicate that keeps a failed removal out of the count driving the CLI's destructive confirmation. |
