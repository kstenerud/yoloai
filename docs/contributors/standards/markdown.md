...
- **The contents of `docs/contributors/archive/`.** The archive is frozen: it is kept for archaeology, is not maintained, and its reliability is to be treated as aged and possibly rotted. Conforming a frozen document to a current convention implies someone vouched for it, which is the opposite of what the archive says about itself — and it would put a fresh timestamp on prose nobody re-read. Sweeps stop at the boundary. Two exceptions: its own `README.md` (an index is live navigation, so it carries a header and stays current), and the **`> **ARCHIVED …**` banner** that every archived file carries above whatever else is at its top (D128) — a warning is the one edit that cannot imply someone vouched for the contents, and it is what puts the archive's own caveat on the path of a reader who arrived by grep rather than through the index. Stamp it on arrival; never revise it. See `../archive/README.md`.
