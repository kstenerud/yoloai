...
**Fix:** Stop tracking docker base-image freshness with a host-side marker at all — stamp the build-inputs checksum **onto the image** as a label (`--label yoloai.base.checksum=<sum>`) and read it back via `Runtime.baseImageStale` (`ImageInspect` → `Config.Labels`). The checksum then travels *with* the image, in whatever store holds it, so each local provider (OrbStack, Docker Desktop, …) is judged by its own image with no key to partition — structurally immune to the provider dimension, the `/var/run/docker.sock` provider-switch symlink, and remote daemons alike. An image with no label (built before this scheme) reads as stale and rebuilds once. A rejected earlier patch keyed the host-side marker by the daemon's `docker info .ID`; that fixed docker but leaned on a property that is empty for podman's docker-compat API, so its correctness varied by daemon (clever-hack smell). The host-side marker (`baseImageChecksumPath`/`NeedsBuild`/`RecordBuildChecksum`) remains for **apple** and **containerd**, which are single-store backends where it is *sufficient*. **Corrected 2026-07-29:** this previously read "apple (a Tart VM image, not OCI — can't carry a label)", which describes **tart**, not apple. Apple `container` runs *Linux OCI* images (`runtime/apple/apple.go`: "Linux OCI in per-container VMs", `baseImage` is "the local OCI image"), so nothing about the format prevents a label there. Whether apple's CLI accepts `container build --label` and reports labels on inspect was the question that decides whether the label scheme can cover all four backends — not the image format. **Verified 2026-07-29 on macOS (`container` CLI 1.0.0, commit `ee848e3`): yes to both.** `container build` lists `-l, --label <key=val>`, and `container image inspect <ref>` reports the label at `[<image>].variants[<platform>].config.config.Labels` — two array levels deep, unlike docker's flat `Config.Labels`, so the extraction does not share code even though the label name does. Evidence and the control run are in [DF152](design/findings-unresolved.md). The misattribution mattered: it read as a hard obstacle and was cited as the reason the split is permanent. See [DF152](design/findings-unresolved.md).
