...
- **Severity:** MEDIUM
- **Disposition:** RESOLVED 2026-06-08. Adopted a fixed convention (documented in `standards/cli.md`): **the top level is always a JSON object, never a bare array**; list commands wrap items in a semantically named array field (`{"backends":[…]}`); arrays are always `[]`, never `null`. Added shared helpers `cliutil.WriteJSONList(w, key, items)` (single-key envelope + nil→[]) and `cliutil.EmptyIfNil(s)` (for array fields in hand-built structs). Migrated the five bare-array commands — `system backends`, `system agents`, `extensions list`, `stop`, `destroy` — to envelopes, which also fixed the `null`-empty bug in `system agents`/`extensions list`. The already-enveloped commands (`sandbox list`, `system disk`, `diff --log`, `system check`) were left as-is (they already conform). Tracked in `BREAKING-CHANGES.md` (Unreleased). Decision: always-object over keep-bare-arrays because a bare top-level array can carry neither a top-level error nor future metadata; semantic keys over a generic `items` for self-documentation. Remaining nits (identifier-key naming `name` vs `sandbox`/`source`/`dest`/`target`, per-item `error` vs `message`, `profile list` lacking `--json`) are documented in the style guide as the target convention; their per-command renames are smaller follow-ups, not blockers for the baseline.
