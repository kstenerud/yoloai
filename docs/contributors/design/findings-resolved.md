...
- **Description:** The v3→v4 overlay flatten runs host-side (capture → repopulate → dispose), so the invoking user must read+remove every host-side file the container wrote. (a) The overlay `upper`/`ovlwork` layers are root-owned kernel dirs, and the kernel makes the overlayfs workdir (`ovlwork/work`) mode **0000** — so `DropDisposer`'s `os.RemoveAll` (as uid 1000) couldn't traverse/remove it → dispose failed after the swap. Fix: `reclaimOverlayLayers` chowns **and** chmods (`u+rwX`) the layers to the invoking user via the running container (root) during capture; the capture also runs as root and hands the stage to the host user (the agent user can't `mkdir` the stage on podman). (b) Podman-rootless maps container users to host **subuids** (`karl:100000:65536` → uid 999 = host 100999), so *all* container-written state (`agent-status.json`, `cache/`, `files/`, `logs/`) is subuid-owned mode 0600 — host-unreadable/unremovable. Normalizing that needs a privileged helper (throwaway root container / `podman unshare`), out of scope; instead the migrator surfaces it as a hard **`AuthBlocked`** plan op — `migrate --check`/`--dry-run` lists it (`[✗]` / `blocked:true`) and a real `migrate` refuses the whole run up front ("can't proceed"), sandbox left untouched — with the fix in the message (recover work via `diff`/`apply`, then `destroy` + recreate as `:copy`). `hostUnmanageableReason` (Plan) + `assertHostOwnedState` (apply re-check).
