...
- **Severity:** LOW
- **Disposition:** PARKED (fix-shape now decided in [D62](decisions/README.md); awaits the multi-principal implementation that introduces the parsed `SandboxName` type)
- **Description:** `config.ValidNameRe` = `^[a-zA-Z0-9][a-zA-Z0-9_.-]*$` accepts a trailing separator and consecutive separators (e.g. `my-app-`, `a..b`, `x__y`), and its `*` allows a 1-char name. containerd's identifier validation (`pkg/identifiers/validate.go`, pinned `containerd/v2@v2.2.2`) is stricter: `^[A-Za-z0-9]+(?:[._-](?:[A-Za-z0-9]+))*$` with `maxLength=76` — every separator must be *surrounded* by alphanumerics, so `my-app-` is rejected. Because `InstanceName(name)="yoloai-"+name` becomes **both** the containerd container id **and** the snapshot key (`lifecycle.go:235,246`), a sandbox whose name yoloAI accepts can fail at containerd create time. Pre-existing and independent of multi-principal (surfaced while researching the namespacing budget); Docker's charset is the same family but doesn't reject the trailing separator, so docker/tart users never hit it. **Decided fix ([D62](decisions/README.md)):** the parsed **`SandboxName`** type (`≤56`, containerd-conformant grammar) whose sole constructor enforces the containerd rule, replacing the loose validate-style `ValidateName` (parse-don't-validate) — chosen over patching `ValidNameRe` in place so the grammar is enforced once at the boundary. This is a (minor) breaking change for any existing sandbox name ending in `-`/`.`/`_`. **Related:** [[DF15]] (deferred-findings.md) parks the *convention* drift on this same value (validate-style `ValidateName` vs parse-don't-validate peers); D62 converges both into the one `SandboxName` type rather than patching the regex twice.
- **Pointer:** `internal/config/names.go:14` (`ValidNameRe`); `internal/sandbox/store/paths.go:113` (`ValidateName`); containerd regex at `containerd/v2@v2.2.2/pkg/identifiers/validate.go:34-42`; reasoning in `docs/contributors/design/research/principal-namespacing.md` (Q1)
