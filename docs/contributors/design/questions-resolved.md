> **ABOUTME:** History sink for design/implementation questions that have been answered.
> Companion to questions-unresolved.md, which holds the active working queue.
...
- **Update (2026-06-29):** RESOLVED — GUIDE.md now documents that Codex resume/restart starts a fresh session with no prior conversation state. Commit 9a0cbd88.
...
104. ~~**Should `store.Environment` carry agent payload?**~~ — **Resolved (D85, 2026-06-14):** No. By symmetry with D84's agent-free `ProvisionSpec`, the substrate's `environment.json` persists only substrate facts (enough to re-`Open` itself); the **agent layer persists its own sidecar** (`agent.json`: `AgentType`/`Model`/`HasPrompt` + agent-launch settings) via `store`'s sudo-safe IO + path layout. Rejected the opaque-payload-in-the-substrate-record option (makes the store a generic carrier). Generalizes: each layer persists its own facts; the substrate record sheds all non-substrate fields (`AutoCommitInterval`/`Archetype`/`VscodeTunnel`/`Principal` when their layers are shaped). Versioned reshape (v2→v3). See [substrate-interface.md](substrate-interface.md) / [D85](../decisions/working-notes.md).

105. ~~**Foundation publicity: does `config.Layout`/`HostEnv` become public?**~~ — **Resolved (D85, 2026-06-14):** No — `config` stays internal (foundation, not promoted). The public substrate takes **narrow, explicit inputs resolved once at the edge** — a small substrate-scoped paths value (the ~6 dirs the runtime uses: `SandboxesDir`/`DataDir`/`CacheDir`/`HomeDir`/lock dirs/`CniDir`) + the curated host-tool env — never the fat aggregate, which is the edge's tool for *producing* those inputs. Parse-don't-validate-at-edges applied to the public boundary. Shape-time sub-question (non-blocking): per-purpose env keysets in the backend vs central in internal `HostEnv`. See [D85](../decisions/working-notes.md).
