...
- **`--images` is the remaining unscoped docker/podman category, on a seed-and-flip timer (2026-07-19):** the docker/podman fix scoped the container/volume/network reclaim but left `ImagesPrune(--images)` daemon-wide, because scoping it immediately would strand pre-label images. The base image now carries `com.yoloai.managed` (inherited by profile images), and the switch to a label-scoped `--images` is registered with a 12-month settling period in [deprecations.md](../deprecations.md) so custom-image authors can adopt the label first. So this finding's docker/podman half has two remainders: the labelled-`--images` flip (dated) and the apple backend (mac-deferred).
