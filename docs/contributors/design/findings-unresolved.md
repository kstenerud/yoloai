...
### DF98 — seatbelt hardcoded the instance prefix too, at nine call sites

- **Discovered:** 2026-07-15 · **Workstream:** DF94 Tart lifecycle verification (D117)
- **Severity:** MEDIUM — a principal-scoped Layout broke every seatbelt sandbox path; unreachable without a principal, which is why nothing caught it
- **Disposition:** ADDRESSED-IN-PLACE (D117) — fixed and unit-pinned alongside the tart twin
...
- **Pointer:** `runtime/seatbelt/seatbelt.go` + `runtime/tart/tart.go` (`instancePrefix`/`sandboxName`); `runtime/seatbelt/seatbelt_test.go` + `runtime/tart/tart_test.go` (`TestSandboxNameIsPrincipalScoped`); `internal/orchestrator/launch/launch.go:146`; `client.go:124`; DF94.
