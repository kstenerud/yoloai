...
### DF28 — Kata conformance/integration suite is flaky under back-to-back VM churn (guest-readiness race)

- **Discovered:** 2026-06-11 · **Workstream:** testing-refactor (verifying DF18's containerd conformance closure on a real Linux+KVM host)
- **Severity:** LOW (test-infra; non-deterministic. No production impact established — the same readiness gap exists in real `create`/`exec`, but is masked by the network probe's best-effort proceed and by warm, non-contended single-instance use.)
- **Disposition:** PARKED
- **Description:** `TestContainerdConformance` (and the bespoke `TestIntegration_ContainerLifecycle`) **run and pass** when subtests run in isolation, but the **full suite is flaky**: a *random* ~2–4 of the Kata-VM-backed subtests fail per full run (three observed runs: FAIL / PASS / FAIL). The failing set is not stable run-to-run, so it is not a per-subtest contract bug. Two signatures, both guest-readiness races *beyond* DF8's TC-filter gap: **(a)** the network probe logs `probe exec create: task <name> not found`, times out after 30s, and a follow-on `Exec` sees `instance not running` — the Kata task isn't yet queryable when the probe execs into it; **(b)** interactive/TTY exec fails with kata-agent `ENOENT: No such file or directory` at exec-process start — the guest rootfs/PATH/agent isn't fully up. Every Kata instance boots a **full systemd guest** (confirmed in journald), and the harness does Create→Start→(probe/exec) immediately; as DF8 notes, `task.Start` returns before the guest is ready. Under 6-core contention with overlapping qemu boots+teardowns the readiness windows widen enough to fail ~10–30% of the time. **Ruled out resource exhaustion:** 6.8 GB RAM free, 25 GB disk free, no leftover tasks at failure time. Distinct from DF8 (route=ok/dns=fail TC-filter gap) — this is task-object and guest-process readiness, a different layer of the same warm-up family. Observed on Ubuntu 24.04 / kernel 6.8 / Kata 3.28.0 (qemu + systemd guest) / containerd v2.2.2.
- **Fix direction:** gate the first probe/exec on kata-agent/guest readiness (or serialize VM setup with a readiness wait) rather than relying on `task.Start` having returned. Documented as [Kata: guest-readiness race](../backend-idiosyncrasies.md#kata-guest-readiness-race-under-back-to-back-vm-churn-task-not-found--exec-enoent).
- **Pointer:** `internal/runtime/containerd/lifecycle.go` (`waitForNetworkReady` — emits the `task ... not found` probe warning), `internal/runtime/containerd/integration_test.go` (`TestContainerdConformance`, `TestIntegration_ContainerLifecycle`), `internal/runtime/runtimetest/conformance_iface.go` (shared harness).
