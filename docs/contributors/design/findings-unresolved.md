...
### DF70 — host `git apply --unsafe-paths` of a container-generated patch = arbitrary host-file write (path traversal) — VERIFIED, LIVE
...
- **Severity:** CRITICAL (arbitrary host-filesystem write as the host user → RCE via `~/.ssh/authorized_keys`, `~/.gitconfig` `core.fsmonitor`/alias, or `~/.yoloai/profiles/base/Dockerfile` on next build). **Reachable in shipped code today**, not only in the unbuilt migration.
- **Disposition:** ESCALATED — fix on `main` independent of the overlay-retirement work. This is the **apply-side** sibling of DF66/DF67 (C1), which confined only the diff-**generation** side; the host-side **apply** of the resulting (untrusted) patch was declared "sound because `--unsafe-paths` is gated" — that conclusion is **wrong** (`--unsafe-paths` is not path-confining).
- **Description:** The overlay patch is produced by `git diff` run **inside the untrusted, agent-controlled container** (`copyflow/apply.go:495-520`, `generateOverlayPatchForContext` → `execInSandbox`), so its bytes are attacker-controlled (the agent has passwordless sudo and can shadow `git`). That patch is applied **host-side** by `git.NewHost(...).ApplyPatch(...)`; for a **non-git** target (`:overlay`/`:copy` dirs aren't repos) it routes through `git apply --unsafe-paths --directory=<target>` (`internal/git/ops.go:198-203`). The apply runs from a throwaway temp repo created **elsewhere** (`withTempGitDir`, `ops.go:414-430`) and redirects writes into the real target via `--directory=`; because the target is outside that temp repo's worktree, git's "refuse to write outside the working tree" guard — the very check that rejects `../`, absolute paths, `.git/`, and beyond-symlink writes — would fire, so **`--unsafe-paths` is added to switch it off**. A `../`+absolute patch path then writes anywhere the host user can. The in-code comment at `ops.go:189-197` states the premise that makes it "safe" (patches "produced by our own git diff over the target tree … never an agent-supplied raw patch") — **false for the overlay path.** Same `--unsafe-paths` branch is shared by copy-mode `ApplyAll`'s non-git target; its patch is host-generated by real git over a real tree (paths repo-relative), so likely safe, but should be confirmed by the same fix.
- **Fix (recipe corrected after empirical re-audit):** `--unsafe-paths` is **not fundamentally
  needed** — it is an artifact of applying from a temp repo *elsewhere* + `--directory=`. The
  load-bearing lever is **run `git apply` with `cwd = <target>`** (git-dir pointed at a throwaway
  temp to avoid ambient repo discovery / `.git` pollution): `git --git-dir=<temp>/.git apply
  <patch>` executed with the working directory set to the target — **no `--directory`, no
  `--unsafe-paths`, and NOT `--work-tree`**. (Verified on git 2.43.0: `git apply` writes relative
  to **CWD**, so `--work-tree` is *inert* — a fix that only adds `--work-tree` while leaving
  `cwd=tmpDir` applies into the throwaway temp and silently lands **nothing** in the target, exit
  0. And keeping `--directory=<abs target>` while merely dropping `--unsafe-paths` breaks even
  legitimate in-tree paths as `invalid path`. So the temp-repo/`runGitApply(ctx, tmpDir, …)`
  structure must change to run at the target.) With `cwd=target` and no `--unsafe-paths`, git
  enforces its native confinement (`../`/absolute/`.git/`/beyond-symlink all rejected — verified)
  and leaves no `.git` in the target (git-dir is the external temp). Apply to **both** `ApplyPatch`
  (`ops.go:198-203`) and `CheckPatch` (`ops.go:160-167`). One `ops.go` change covers **both** the
  overlay apply and copy-mode's non-git `ApplyAll` branch — both reach it via `IsGitRepo(hostPath)`
  == false. Prefer letting real git enforce confinement over hand-rolled `..`-rejection. The
  treat-the-patch-as-untrusted boundary (D86 hermetic seal) is correct; only the apply was
  under-hardened.
- **Pointer:** `internal/git/ops.go:160-204` (`CheckPatch`/`ApplyPatch` `--unsafe-paths` branches + the false-premise comment), `ops.go:414-430` (`withTempGitDir`), `copyflow/apply_overlay.go:64-68` (the host apply call), `copyflow/apply.go:495-520` (in-container diff gen); refutes the soundness claim in [plans/copy-mode-git-rce.md](plans/copy-mode-git-rce.md) (C1/DF66).
