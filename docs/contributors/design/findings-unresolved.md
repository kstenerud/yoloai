...
- **Severity:** LOW–MEDIUM (a profile sandbox keeps running on a base that has since changed; no error, and the drift is silent — but the base changes rarely, and `--rebuild` fixes it once noticed)
...
- **What to weigh when picking it up:** the honest question is not whether the drift is real (it is) but whether a silent stale base is worse than a loud unexpected rebuild. A middle option exists: fold the base checksum in, but only warn on mismatch rather than rebuilding, letting the user choose. Note also that the base's own staleness IS tracked correctly (label on docker/podman, keyed marker on apple/containerd), so the information needed is already at hand — nothing has to be plumbed.
