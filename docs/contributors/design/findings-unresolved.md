...
- **Fix:** `--unsafe-paths` is **not fundamentally needed** — it is an artifact of applying from a temp repo *elsewhere* + `--directory=`. Make the **target itself the work-tree** with metadata kept out of it: `git --git-dir=<temp>/.git --work-tree=<target> apply <patch>` (no `--directory`, no `--unsafe-paths`). git then enforces its native path confinement (`../`/absolute/`.git/`/beyond-symlink all rejected — reviewer's in-worktree tests confirm) while leaving no `.git` in the non-repo target. Apply to both `ApplyPatch` and `CheckPatch` (`ops.go:160-167` also uses `--unsafe-paths`). Prefer letting real git enforce confinement over hand-rolled `..`-string rejection. The treat-the-patch-as-untrusted boundary (D86 hermetic seal) is correct; only the apply was under-hardened.
