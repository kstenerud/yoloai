...
- **Three ways out, and the third is the one worth arguing about:**
  ...
  - **Stop baking the scripts in.** The skew exists only because the runtime scripts are *copied* into every image and then diverge from the host that drives them. Delivering `/yoloai/bin` at launch instead — the host's embedded copy, always current — deletes this class rather than detecting it, and needs no rebuild at all. It is the same move DF152 made for staleness: one source of truth instead of a copy that can rot. It is also the most invasive: VM backends have no host bind mount to lean on, and it changes how self-contained an image is. Worth pricing before assuming rebuild-on-mismatch is the answer.
