...
- **Disposition:** PARKED — **design converged + independently audited (D110)**; reworked plan ([plans/crash-safe-migration.md](plans/crash-safe-migration.md)) + audit ([plans/crash-safe-migration-audit.md](plans/crash-safe-migration-audit.md), A1–A18). The stamp-before-pass bug itself is dissolved by stamp-last + per-sandbox-version-as-truth; **open for further critique, not yet built.** Note the audit also confirmed (in code) sibling-finding **A1** (no durable write primitive) and **A4/C3** (the existing split is power-loss-lossy) — both must be fixed in 0.6.0.
