...
- **Disposition:** PARKED
- **Rides:** a **breaking** release — *the rejection only.* Making `-p` an error is newly-rejected input (rule 1), so it is cheap only while a release already breaks. Wiring `--net-softnet` port-forwarding is a feature, needs real macOS verification, and is not release-gated.
