...
### DF69 — macOS overlay upper is tmpfs-only (live-or-lose); flatten must convert while running — CONFIRMED
...
- **Disposition:** VERIFIED — confirmed by live test; **recorded in [`backend-idiosyncrasies.md`](backend-idiosyncrasies.md)** (macOS overlayfs/VirtioFS tmpfs-upper entry) and shapes the migration-version's overlay→copy flatten. Loss-on-restart reproduced; the "graceful shutdown syncs tmpfs→host" escape hatch was **refuted** (no sync).
...
- **Disposition:** PARKED — **design converged + independently audited (D110)**; reworked plan ([plans/crash-safe-migration.md](plans/crash-safe-migration.md)) + audit ([plans/crash-safe-migration-audit.md](plans/crash-safe-migration-audit.md), A1–A18). The stamp-before-pass bug itself is dissolved by stamp-last + per-sandbox-on-disk-form-as-truth (a single data-dir schema chain; the sub-realm idea was dropped 2026-06-30); **open for further critique, not yet built.** Note the audit also confirmed (in code) sibling-finding **A1** (no durable write primitive) and **A4/C3** (the existing split is power-loss-lossy) — both are exposures in the **sealed v2→v3 split**; the v3→v4 machinery fixes the class going forward, while retro-hardening the existing split is an open call.
