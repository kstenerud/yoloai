...
- **Discovered:** 2026-06-29 · **Workstream:** disk-reclaim / prune evaluation
- **Severity:** MEDIUM (leaves tens of GB reclaimable + misleads the user about how much was freed) · **Disposition:** PARKED
...
- **Update (2026-06-29) — reporting-accuracy parts of #2/#3 addressed:** containerd's `pruneSnapshots` now **excludes devmapper bytes from the reclaimed total** (they return to the thin-pool but free no host disk — counting them was an over-report) and reports them on their own line; and `PruneCache` now prints a **per-backend `containerd: reclaimed X` line** like docker/podman so its contribution is visible instead of silently folded into the aggregate. Also factored a shared `runtime.FormatBytes`. **Still open:** (1) the stale-container hard-block; (2) actually returning devmapper space to the host (pool discard/recreate — the manual recipe is in [[reference_disk_reclaim_recipes]]); and the larger **(c) content-store accounting** — `pruneImages` frees the content-store blobs but returns 0 bytes, and `CacheUsage` likewise sums only snapshot `Usage` (never the content store), so both prune-reclaim and doctor-sizing still miss that tier, and socket `Usage()` reads low for committed overlayfs snapshots without the host-fs `du` the design deliberately avoids.
- **Pointer:** `runtime/containerd/prune.go` (`PruneCache`, the still-exists abort + the now-split snapshot/devmapper accounting), `runtime/format.go` (`FormatBytes`), `internal/.../system prune` reclaim-total accounting; verified live on this host (32 GB containerd store, 11 GB devmapper data file unshrunk after `--images`).
