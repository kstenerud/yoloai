...
- **Update (2026-06-29) — code fix applied on branch `release-prep`, UNVERIFIED:** both launches now use `SysProcAttr{Setsid: true}` (new session, no controlling tty) instead of `Setpgid: true`, mirroring the broker sidecar's detach idiom (`internal/broker/host.go:160`). Verified on Linux: the change builds + darwin-crosschecks, and the teardown path is unaffected (`Stop` does `kill(-pid)` group-kill; the Setsid child remains its own process-group leader, so the group is still reaped). **Still PARKED — NOT yet verified on macOS:** the finding's open risk ("does tmux still start cleanly under the new session on seatbelt and tart?") can only be checked on a macOS host. Do not close until a macOS run confirms tmux starts and the controlling terminal is no longer corrupted by a non-interactive `new`/`start`.
