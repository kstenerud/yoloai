...
- **Round two (2026-07-27), measured only on what the review did NOT name:** every structural fix held. Rule 7 — the one rule ignored in round one despite sitting in the shortest document in the repo — produced DF153, filed unprompted. The `var _ runtime.ProfileImageBuilder` assertion nobody mentioned was added. No archived doc was mined as a spec, and both stale paths were corrected. An argv test appeared carrying rule 10's reverted-and-checked verification. **One miss, and it is the informative one:** a second behaviour change in the same PR (which environment the build subprocess draws from) went untested, and reverting it left the whole suite green. Rule 10 named argv as its example, argv was tested, and the rule was followed exactly as worded — so the wording was the defect, not the compliance. Paired with round one's archive failure, that is the same shape twice: correct content, narrower reading than intended. Named as GEN §17 and remediated in D129.
