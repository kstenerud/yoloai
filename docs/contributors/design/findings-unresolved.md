...
- **Disposition:** ADDRESSED-IN-PLACE — detection + directive message shipped in `yoloai doctor` (`runtime/tart/netcheck.go`, `runtime.NetLivenessReporter`), verified live against the wedged VM (`tart ip` empty + guest `en0` `169.254.93.37` → `network: WEDGED`, exit 1). Idiosyncrasies entry extended (scope escalation + new symptom-index row). Remaining surfacing (info/ls status, smoke fail-fast) tracked in `design/plans/tart-network-liveness.md`.
