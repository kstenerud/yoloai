...
### DF100 — two different gates in `repo_hygiene_test.go` both call themselves "Gate C"

- **Discovered:** 2026-07-15 · **Workstream:** the D119-D121 agent-error gates plan (`design/plans/agent-error-gates.md`), while allocating a letter for Gate D
- **Severity:** LOW (a naming collision in test-file prose; nothing miscompiles, and both gates run)
- **Disposition:** PARKED — filed rather than fixed, because renumbering a gate is a rename sweep (AGENTS.md rule 2) and the plan that surfaced it owns neither gate.
- **Description:** `TestRepoHygiene_TestGates_AreSetBySomething` introduces itself as "Gate C" (`repo_hygiene_test.go:714`), and so does the complexity-suppression pair — the section header at `:913`, then "Gate C part 1" at `:947` and "Gate C part 2" at `:985`. So `Gate C` resolves to two unrelated gates, and the file's own scope log prints "Gate C scope:" for the env-gate one only. `findings-unresolved.md:397` cites "D119 Gate C (test-gate liveness)", which pins the name to the env-gate test and leaves the complexity pair as the mislabelled one. This is the duplicate-ID class of D121 and `scripts/next-id.sh` one level up: the letters are allocated by reading the file for the highest, and the section headers (A at `:227`, B at `:410`, C at `:913`) disagree with the per-test labels. Gate D is unaffected — D is free under either reading — so it landed as planned.
- **Pointer:** `repo_hygiene_test.go:714` vs `:913`/`:947`/`:985`; `docs/contributors/design/findings-unresolved.md:397` (the external citation that pins the name)
...
- **Discovered:** 2026-07-15 · **Workstream:** D119 Gate C (test-gate liveness), which surfaced it before it was even written
