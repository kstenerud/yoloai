...
- **Measured on this machine (macOS, 2026-07-30), not predicted.** Moving the target to `/tmp/yoloai-conformance-mnt-{rw,ro}` and exec'ing through `ResolveGuestMountPathFor` un-skips seatbelt: `Mounts/ReadWrite` **passes**. Docker's `Mounts` passes unchanged under the same edit, so the indirection costs the container backends nothing. **Tart was not run** — it needs `YOLOAI_TEST_TART_VM=1` and clones a multi-GB VM per subtest; its half is inferred from the resolver and remains unverified.
