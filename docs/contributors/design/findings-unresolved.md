...
### DF21 — Host-global base-image staleness marker can leave a second daemon on a stale image
...
- **Severity:** LOW
- **Disposition:** PARKED
- **Correction — the original "SDK can't see the containerd-store image" diagnosis was WRONG (verified by diagnostic).** A throwaway integration test against the live Docker Desktop (containerd image store, `io.containerd.snapshotter.v1`) socket showed the Go SDK works fine: `imageExists("yoloai-base")` → **true**, `ImageInspect` → ok (not NotFound), `ImageList(reference=yoloai-base)` → 1 image (`yoloai-base:latest`). So `imageExists` needs **no** change. The *repeated* Docker Desktop rebuilds were the base image **build failing** — the transient VS Code CDN 404 with no retry — so no image ever landed and `Setup` took the `!exists` path on every run. That is **already fixed** by `7335018` (fix(docker): retry base-image downloads); once the build succeeds the image persists and the SDK finds it, so the loop stops.
- **Remaining (real, minor) bug:** the staleness marker `.base-image-checksum` is **host-global** (`baseImageChecksumPath` → `CacheDir()`, one per DataDir) while images are **per-daemon**. After a Dockerfile change, the first daemon to rebuild records the shared marker; a second daemon that already has an image then skips `NeedsBuild` (`docker.go:321`) and silently keeps a **stale (pre-change)** image. Niche — multi-daemon hosts only — but a real correctness gap.
- **Fix:** record the build-inputs checksum as an **image label** read per-daemon from the image itself (presence + currency in one inspect), or key the marker per daemon endpoint — so a Dockerfile change forces every daemon's image to refresh.
- **Trigger:** a reported stale-image-after-Dockerfile-change on a multi-daemon host, or next time we touch the docker build/ready path.
- **Pointer:** `internal/runtime/docker/build.go` (`NeedsBuild`, `baseImageChecksumPath`, `RecordBuildChecksum`), `internal/runtime/docker/docker.go:309/321` (Setup gate).
