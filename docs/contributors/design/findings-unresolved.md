...
### DF150 — the profile build-staleness marker is not backend-keyed, so alternating backends skips a build that never happened

- **Discovered:** 2026-07-27 · **Workstream:** PR #44 review (first external contribution)
- **Severity:** MEDIUM (a failed run with a confusing error — an image pull for a local-only tag — not data loss; but it reproduces on demand and the error names nothing useful)
- **Disposition:** PARKED
- **Description:** `runtime/docker/build.go:27` defines `lastBuildFile = ".last-build-checksum"`, written unqualified into the profile directory. The profile directory is shared across backends; the **image stores are not**. Build profile `dev` under docker, then run the same profile under `--backend podman`: `ProfileImageNeedsBuild` finds a matching checksum, returns false, podman skips the build, and `podman run yoloai-cli-dev` fails trying to pull a tag that only ever existed in docker's store. It fails in both directions and, once apple grows profile-image support (PR #44), on macOS too — where `apple` is the default when installed, so users alternate constantly. **Verified by inspection, not by running it:** the marker is unqualified at `build.go:27`, `profileBuildChecksum` (`:357`) takes only `profileDir`, and docker/podman keep separate stores. The reproduction above is therefore predicted, not observed; running it costs one profile build and would settle it.
- **The fix already exists one function away.** The *base* image marker solved exactly this: `baseImageChecksumPath(layout, backendKey)` → `.base-image-checksum-<backendKey>` (`build.go:52`), with a comment at `:56-59` stating that it must be per-store and naming the four stores. The profile path needs the same key threaded through `ProfileImageNeedsBuild` / `RecordProfileBuildChecksum` → `.last-build-checksum-<backendKey>`. Both are also worth extracting: they are one scheme with two consumers, currently duplicated.
- **Why it survived:** the base-image half was fixed, reasoned inline, and the reasoning was recorded in `findings-resolved.md:929` — a sink nothing sweeps and no router points at. Nothing carried the invariant to the second consumer, and nothing states it where a contributor would meet it. Now stated in `architecture/host-layout.md` → "Build-staleness markers are keyed by backend" and on the `runtime.ProfileImageBuilder` contract. See [DF151](#df151--the-documentation-system-routes-agents-to-archived-plans-and-hides-its-conventions-in-append-only-sinks).
- **Pointer:** `runtime/docker/build.go:27` (`lastBuildFile`), `:52` (`baseImageChecksumPath`, the model), `:357` (`profileBuildChecksum`); `runtime/runtime_optional.go` (`ProfileImageBuilder`).
