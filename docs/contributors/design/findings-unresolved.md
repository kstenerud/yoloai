...
- **What the pin actually rests on.** `bca8af21` (2026-03-17): *"Node.js 22 has syscall incompatibilities with gVisor ARM64 that cause silent immediate crashes with zero output, killing the tmux session before remain-on-exit fires."* Read the same commit's other change and the claim reads differently: it also added a `node --version` probe logged as `sandbox.node_check` *"so gVisor syscall rejections surface immediately"* — i.e. the diagnostic was introduced **because the cause could not be seen**. "Syscall incompatibility" was the hypothesis that motivated the downgrade, not an observation that survived it. The observation was a silent crash.
...
- **Absence of reports is weak evidence here**, and worth saying so rather than reading it as vindication: gVisor on ARM64 is a thin population, and this repo's combination (gVisor + ARM64 + Node under tmux) is thinner still. The honest position is that the claim is neither confirmed nor refuted.
...
- **The decision, stated plainly:** we are holding every user on an EOL Node, and permanently capping Claude Code, to avoid a failure mode that was never reproduced, never diagnosed past a symptom, and has no public counterpart — on one architecture, in one isolation mode (`container-enhanced`), which is not the default. Options: (1) retest on ARM64 and move to Node 22 if clean; (2) move to Node 22 regardless and treat gVisor-on-ARM64 as a known-risk combination, which the existing `sandbox.node_check` probe already instruments; (3) keep Node 20 and accept that Claude Code is frozen, which is not viable for long. **Not decided here** — it is an owner call with a real if unquantified risk on one side and a certain, growing cost on the other.
