...
### DF126 — profile image tags carry no principal, so two principals with a same-named profile collide on one image
...
- **Disposition:** FILED, not fixed — **and this release is the last cheap moment.** The fix renames a user-visible image tag (`yoloai-web` → `yoloai-cli-web`), so it is breaking; v0.9.0 already breaks on exactly this prefix and already ships a `system migrate`, so landing it here costs users nothing they are not already paying. Landing it later costs a second break of the same prefix, a second BREAKING-CHANGES entry saying the same sentence, and — because `ImageRef` is persisted (see below) — plausibly a schema v6 and a second migration. Since the defect is latent, it could never justify that bill on its own, which is precisely how a gap like this survives forever.
- **Description:** D62 scoped instance names by principal and left image tags unscoped; D126 scoped the CLI but did not reach tags. Three sites build the tag by hardcoding the prefix, none of them via `config.InstancePrefix`:
  - `internal/config/profile.go:274` — `ResolveProfileImage` returns `"yoloai-" + name` (the read side), falling back to `"yoloai-base"`.
  ...
  So two principals each holding a profile named `web` both build and resolve `yoloai-web` in a shared daemon; the second build wins and the first principal's sandboxes silently run it.
- **This is the DF98 class exactly one artifact over, and D126's poka-yoke does not cover it.** D126 made `InstancePrefix` incapable of emitting a bare `"yoloai-"` (it panics on the empty principal) precisely so the hardcode could not be written. These three sites never call it, so they hardcode the string directly and the gate cannot see them. That is worth stating plainly: the poka-yoke guards the *function*, not the *string*, which is the limit DF98 already recorded when a lexical ban on the prefix was rejected.
- **It is not a pure cache rename — it touches persisted state.** `store.Environment.ImageRef` (`store/environment.go:39`) records the resolved tag per sandbox and is read on the launch/restart path (`launch.go:355,877`; `lifecycle/restart.go:197`), with a legacy backfill to `"yoloai-base"` (`environment.go:184-188`). So a rename must also re-stamp existing records, or restart resolves a tag that a rebuild has orphaned — which is why "just rebuild the image" understates it, and why doing it after the migration ships is materially more expensive than doing it inside it.
- **Scope note.** `yoloai-base` is yoloAI's own artifact, byte-identical for every principal, and sharing it is desirable (one copy, one build). Only *principal-authored* profile images need scoping — which is what `research/principal-isolation.md` §B2 recommends: *"any principal-authored build artifact (custom profile/image) lives in the principal partition with a principal-scoped image tag."* B2's wider question (may principals author profiles at all?) stays the embedder's, per D126(4).
- **Pointer:** `internal/config/profile.go:262-278` (`ResolveProfileImage`); `internal/orchestrator/profiles/profile_build.go:74`; `profile.go:274`; `store/environment.go:39,184-188` (`ImageRef` persisted + backfilled); `internal/orchestrator/launch/launch.go:355,877`; `internal/orchestrator/lifecycle/restart.go:197`; `internal/config/names.go` (`InstancePrefix`, the gate these sites bypass); `research/principal-isolation.md` §B2; D62; D126(4); DF98; [cli-is-a-principal.md](../archive/plans/cli-is-a-principal.md) open question 4.
...
- **Disposition:** FILED, not fixed. The fix is to make the gate more helpful, not to make migrate more paranoid — which is the reverse of what the original filing implied, and the reason it is worth stating explicitly.
