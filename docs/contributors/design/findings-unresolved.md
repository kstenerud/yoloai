...
### DF158 — nothing declares or checks a minimum agent-CLI version, so a pinned agent survives every rebuild

- **Discovered:** 2026-07-30, separating out what [DF156](findings-resolved.md) did *not* fix · **Workstream:** profile-image lifecycle
- **Severity:** LOW–MEDIUM. Needs a user who pins an agent CLI and a yoloAI release that starts using a newer flag — but the symptom is the agent behaving oddly, not a legible error, which is what raises it above LOW.
- **Rides:** any.
- **Why it is not DF156.** That finding was about images going stale against the base, and its remedies all work by *rebuilding*. This one is immune to rebuilding: a profile Dockerfile that installs `claude@1.2.3` reinstalls exactly that on every rebuild, however current the base becomes. The two look alike — both are host↔guest version skew — and DF156 mentioned this in passing, which is precisely why it needs its own entry rather than a line inside a resolved one.
- **What is missing.** `internal/agent/agent.go` hardcodes each agent's flag surface (`ModelFlag`, resume flags, `HeadlessCmd`, the settings-file mechanism), and the `Definition` struct carries **no minimum version** for the CLI it drives. Nothing queries the installed version in the sandbox, and nothing compares one. So a host that outruns the agent in the image fails at the point the unknown flag is passed, inside the agent's own output, where it reads as the agent misbehaving.
- **The precedent for the fix is in-tree and worth copying rather than inventing.** `RUNTIME_CONFIG_SCHEMA_VERSION` (`runtime/monitor/setup_helpers.py`) versus `runtimeconfig.SchemaVersion`, held together by `TestSchemaVersion_GoPythonAgreement`, is the same problem solved for the config format: a declared number on each side and a cross-language test that they agree. The gap is that it covers the config *format* and not the agent CLIs.
- **Unknowns to settle before building.** Whether a floor can be checked cheaply — each CLI has its own `--version` output to parse, which is a per-agent probe and a per-launch cost — and whether the answer is a floor at all rather than capability detection, since "supports `--foo`" is the actual question and version numbers are only a proxy for it. Also unclear whether this should fail the launch or warn: for a pinned agent the user made a deliberate choice, and refusing to launch would override it.
- **Pointer:** `internal/agent/agent.go` (the `Definition` table and its flag fields); `runtime/monitor/setup_helpers.py` + `internal/orchestrator/runtimeconfig` (the schema-version precedent).
