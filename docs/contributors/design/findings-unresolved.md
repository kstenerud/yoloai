...
### DF21 — Docker Desktop containerd image store: SDK `imageExists` misses `yoloai-base`, forcing a full rebuild every run

- **Discovered:** 2026-06-10 · **Workstream:** Apple `container` backend (side-discovery while diagnosing repeated base-image rebuilds during `make smoketest-full`)
...
- **Disposition:** PARKED
- **Description:** `docker.Runtime.Setup` (`docker.go:309/314`) rebuilds when `!imageExists("yoloai-base")`; `imageExists` (`docker.go:335`) calls the Go SDK `client.ImageInspect`. Against **Docker Desktop's containerd image store** (`Storage Driver: overlayfs`, `driver-type: io.containerd.snapshotter.v1`), `ImageInspect("yoloai-base")` returns NotFound **even though** `docker --context desktop-linux image inspect yoloai-base[:latest]` finds it (`sha256:752ab8…`, arm64). So Setup takes the `!exists` path ("Building base image (first run only)…") and rebuilds from `FROM debian` on **every** run. OrbStack (classic `overlay2`) is unaffected — the only difference is the daemon's image store. The client already negotiates the API version (`docker.go:153 WithAPIVersionNegotiation`), so it is not a stale-API-version issue; the likely mechanism is a short-name/ref-resolution difference between the SDK `/images/{name}/json` path and the CLI against the containerd store. Reproduced 2026-06-10 (smoke run `…115124` desktop-linux rebuilt; orbstack the same run reported "Base image built successfully" = skipped). **Not caused by the apple/probe/Dockerfile work** — pre-existing docker-backend behavior, surfaced by Docker Desktop's containerd store.
- **Secondary bug (same code path):** the staleness marker `.base-image-checksum` is **host-global** (`baseImageChecksumPath` → `CacheDir()`, one per DataDir) while images are **per-daemon**. After a Dockerfile change, only the *first* daemon to rebuild updates the shared marker; other daemons that already have an image then skip `NeedsBuild` (`docker.go:321`) and silently keep a **stale image**.
- **Fix:** make image-presence containerd-store-robust — e.g. `ImageList` with a `reference=yoloai-base` filter, or try the full ref `yoloai-base:latest`, or fall back to a CLI `docker image inspect` shell-out when the SDK returns NotFound (verify against both an `overlay2` and a containerd-store daemon). Better: record the build-inputs checksum as an **image label** read per-daemon from the image itself — that fixes *both* the containerd-store visibility (presence + currency in one inspect) and the host-global-marker staleness, and survives the harness's DataDir handling. Add a `backend-idiosyncrasies.md` entry (symptom: "Docker Desktop rebuilds yoloai-base every run") when fixed.
- **Workaround:** disable Docker Desktop → Settings → "Use containerd for pulling and storing images" (reverts to overlay2), or use OrbStack.
- **Trigger:** next time we touch the docker build/ready path, or when the containerd image store becomes unavoidable (newer Docker Desktop defaults). Validate the chosen presence check on both store types.
- **Pointer:** `internal/runtime/docker/docker.go:335` (`imageExists`), `:309/:314/:321` (Setup gate + `NeedsBuild` branch), `internal/runtime/docker/build.go` (`NeedsBuild`, `baseImageChecksumPath`, `RecordBuildChecksum`).
