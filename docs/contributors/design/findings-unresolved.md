...
- **Sequenced in** [`plans/profile-image-lifecycle.md`](plans/profile-image-lifecycle.md) — build **after** DF154's recovery half, not before: a label read is a better proxy than a filename but still check-then-act, so it is only safe to add once a missing image at launch is recoverable rather than fatal.
