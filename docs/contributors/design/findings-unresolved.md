...
- **Trigger / fix:** already implemented this pass. Follow-up trigger: pin the exact crun version that fixes CVE-2026-47766 (search returned the CVE and its fix description but not an isolated version tag) and tighten the crun floor from 1.20 to that version once confirmed.
...
- **Disposition:** PARKED
- **Description:** `internal/broker` and `internal/credential` (D105/D106) were built explicitly general — "Keep the broker GENERAL — not Claude/LLM-specific" per the decision log — with a closed-set `CredentialSource`/`Apply`/`Destination` model designed to cover more than one provider. In practice, `internal/agent/agent.go` only populates `Broker: &BrokerConfig{...}` for the `"claude"` definition (`UpstreamURL: "https://api.anthropic.com"`, `Destination`, per-credential `EnvVar`/`Header`/`Prefix`, `BaseURLEnvVar`, `AuthTokenEnvVar`, `DummyToken`). The `"gemini"`, `"codex"`, and `"opencode"` definitions have no `Broker` field at all, so their API keys are delivered directly into the sandbox (file-based injection, not brokered) even though the architecture supports it.
- **Trigger / fix:** wire a `BrokerConfig` for each remaining agent, same shape as Claude's: `gemini` → `UpstreamURL: "https://generativelanguage.googleapis.com"` (also covers `cloudcode-pa.googleapis.com`/`oauth2.googleapis.com` per its existing `NetworkAllowlist`); `codex` → `UpstreamURL: "https://api.openai.com"`; `opencode` is multi-provider (`APIKeyEnvVars` list) and needs a `BrokerConfig` per configured provider, the most design work of the three. Revive when someone picks up broker-generalization work, or when a user requests brokering for a non-Claude agent.
- **Pointer:** `internal/agent/agent.go` (lines ~236–478, per-agent `Definition`s); `internal/broker/`; `internal/credential/`; `docs/contributors/decisions/working-notes.md` (D105, D106).
...
- **Disposition:** ADDRESSED-IN-PLACE for the CVE question (verified, no action needed); ESCALATED-then-PARKED for the install-hint defect (real but out of scope for this pass)
- **Description:** Two CVE IDs surfaced in earlier secondary-source research (CVE-2026-5747, CVE-2026-1386) were verified against NVD and the firecracker-microvm GitHub security advisories directly. Both are real, both are genuine Firecracker VMM vulnerabilities: **CVE-2026-5747** (GHSA-776c-mpj7-jm3r, virtio-pci OOB write, opt-in `--enable-pci` only, CVSS 7.5–8.7, affects 1.13.0–1.14.3 and 1.15.0, fixed in 1.14.4/1.15.1) and **CVE-2026-1386** (GHSA-36j2-f825-qvgc, jailer symlink-following arbitrary host-file overwrite, CVSS 6.0, affects ≤1.13.1 and 1.14.0, fixed in 1.13.2/1.14.1). Kata Containers' own `versions.yaml` (main branch) pins `assets.hypervisor.firecracker.version: v1.12.1` — below the affected range of both CVEs, so yoloAI's `vm-enhanced` mode has no current exposure to either. Kata itself has six unrelated 2025-2026 CVEs, but the two hypervisor-specific ones (CVE-2026-24834, CVE-2026-47243) affect Cloud Hypervisor/QEMU, not Firecracker. **Separately:** the install hint in `runtime/containerd/caps.go` (`buildKataShimV2Cap`, `buildKataFCShimV2Cap`) and `containerd.go` — `sudo apt install kata-containers` — does not correspond to any currently maintained package; Kata's apt/OBS repo was archived in 2021 and only ever covered the legacy 1.x series. Current Kata (2.x/3.x, latest v3.32.0) installs via `kata-manager.sh` or `kata-deploy`, both of which resolve to whatever `versions.yaml` pins.
- **Trigger / fix:** CVE question needs no action now — revisit if Kata's `versions.yaml` ever bumps Firecracker into the 1.13.0–1.15.0 range without also picking up 1.14.4/1.15.1/1.13.2/1.14.1. Install-hint fix: replace the `sudo apt install kata-containers` `Fix` steps in `runtime/containerd/caps.go` (two call sites) and the `InstallHint` in `containerd.go` with the `kata-manager.sh`/`kata-deploy` method.
- **Pointer:** `runtime/containerd/caps.go` (`buildKataShimV2Cap`, `buildKataFCShimV2Cap`); `runtime/containerd/containerd.go` (`InstallHint`); upstream `kata-containers/kata-containers` `versions.yaml`.
