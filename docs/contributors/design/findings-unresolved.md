...
- **Disposition:** PARKED
- **Description:** Codex CLI 0.144 does **not** authenticate from a bare `OPENAI_API_KEY`/`CODEX_API_KEY` environment variable — `codex login status` reports "Not logged in" and the TUI prompts for login (verified empirically, see `backend-idiosyncrasies.md` → "Agent CLIs: base-URL override for brokering differs per CLI"). It reads a logged-in `~/.codex/auth.json` (`{"auth_mode":"apikey","OPENAI_API_KEY":<key>}`). yoloAI's **direct** (non-brokered) credential delivery hands Codex the key as an env var / `/run/secrets` file and seeds `auth.json` **only** from a host `~/.codex/auth.json` (an `AuthOnly` seed, skipped when an API key is present). So a user with an `OPENAI_API_KEY` but no prior host `codex login` gets a Codex sandbox that immediately asks to log in. D115's **brokered** path now generates a working placeholder `auth.json`, so default (brokered) Codex works — but `--no-broker` Codex, or Codex on a backend that can't host an injector, still hits this.
- **Trigger / fix:** for direct-delivery Codex with an API key present and no seeded `auth.json`, generate `~/.codex/auth.json` = `{"auth_mode":"apikey","OPENAI_API_KEY":<real-key>}` from the resolved secret (the non-brokered analogue of `agent.patchCodexAuth`). Revive when someone touches direct credential delivery, or when a user reports `--no-broker` Codex asking to log in.
- **Pointer:** `internal/agent/agent.go` (codex `SeedFiles` `auth.json` `AuthOnly`), `internal/envsetup/` (direct secret/seed delivery), `internal/agent/broker_codex.go` (`patchCodexAuth`, the brokered analogue); `docs/contributors/backend-idiosyncrasies.md` (Codex auth.json entry).
