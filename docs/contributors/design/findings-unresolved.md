...
### DF158 — the base image's agent CLIs are cached indefinitely, so "unpinned" does not mean current

- **Discovered:** 2026-07-30, checking whether a profile Dockerfile could pin an old agent CLI. It can, but that is not the exposure · **Workstream:** profile-image lifecycle
- **Severity:** MEDIUM. Bounded only by how long ago a host first built the npm layer, and unbounded in principle: a machine whose base dates from a year ago runs a year-old CLI with no signal. The Dockerfile's own comment says a stale client "fails against the live API within weeks", which is the failure mode this creates by accident.
- **Rides:** any.
- **The Dockerfile states the requirement and does not achieve it.** Agent CLIs are installed deliberately unpinned — *"versions intentionally unpinned; agents must run at current versions to function correctly with their respective APIs"*, with DL3016 suppressed and the reasoning spelled out (DF52). That intent is right. But `npm install -g` sits in a `RUN` layer that nothing invalidates: the instruction never changes, so BuildKit reuses it forever, and `buildBaseImage` passes no `--no-cache`. Unpinned buys "latest at the moment this layer was *first* built", which is not what the comment promises.
- **Observed on this host, 2026-07-30.** A base image whose `.Created` is today reports `claude 2.1.197` while npm's `latest` is `2.1.220`. `yoloai system build --rebuild` does not close the gap — `force` re-runs the build, not the layer — and a following build reports the surrounding steps `CACHED`. So "rebuild the base" is not a remedy a user can reach for, and neither is `--rebuild`.
- **This is NOT what DF156's checksum split changed, checked rather than assumed.** The npm layer sits at Dockerfile line ~145, *above* every `COPY … /yoloai/bin/` at 256+. Script changes never busted it, before or after the split, because a lower layer cannot invalidate a higher one. The split changed how often the base rebuilds; it did not change how often the CLIs refresh, which was already never.
- **What this is not.** A *profile* Dockerfile can also install an older CLI — `npm install -g <pkg>@1.2.3` replaces ours in place, so it needs no uninstall and is not the "active sabotage" it sounds like. It is still a deliberate act by the user against a version we installed on purpose, and treating that as their choice rather than our defect is the owner's call (2026-07-30). The finding is about our own image, not theirs.
- **The related half that stays open with it:** `internal/agent/agent.go` hardcodes each agent's flag surface and its `Definition` carries no minimum version, so when the image's CLI *is* too old the failure surfaces inside the agent's output as odd behaviour rather than as a version error. Whether that needs its own guard depends on which remedy below lands: a bounded-staleness refresh makes it rare, a floor makes it legible.
- **Remedies, none obviously right — this is a rebuild-frequency policy question, which is why it is filed rather than fixed:**
  - **Periodic cache bust.** An `ARG` fed something coarse (an ISO week) makes the npm layer re-run on a schedule, bounding staleness at the cost of one multi-minute base rebuild per period. Matches the stated intent most directly.
  - **`--no-cache` on the base build.** Correct and far too expensive: every base build becomes a full one.
  - **Check at launch and warn.** Cheap and needs no rebuild, but each CLI has its own `--version` output to parse, so it is a per-agent probe and a per-launch cost — and it reports a problem it cannot fix.
  - **Declare a floor — verified feasible 2026-07-30, and the strongest of these.** npm accepts an open-ended minimum: `npm install -g "<pkg>@>=X.Y.Z"` resolves to *latest* when the floor is satisfiable (`>=2.1.0` → `2.1.220`) and **errors** when it is not, so it fails closed rather than silently degrading. That makes the Dockerfile itself both the declaration and the enforcement, and raising the floor changes the Dockerfile → changes `buildInputsChecksum` → rebuilds the base and every profile through the machinery DF156 already built. No new mechanism, and no separate marker: an `echo "minimum claude vX.Y.Z"` cache-buster would achieve the rebuild but enforce nothing, because nothing reads it.
    **It must be quoted, and the unquoted form is silently inert** — the failure this would otherwise ship with. Written naively in a `RUN` line, `sh` consumes `>=99.0.0` as a redirection: it creates a file named `=99.0.0`, hands npm the bare package name, and **succeeds**, installing latest against an impossible floor. Demonstrated directly. So the floor would appear to work while enforcing nothing, and the only trace is a stray `=99.0.0` in the build layer. Any implementation needs a test that an unsatisfiable floor *fails* the build.
    **What it does not buy:** anything about drift *above* the floor. Between raises the layer stays cached, so the CLI can fall arbitrarily far behind on the API axis — which is the axis the Dockerfile's own comment is worried about. This remedy addresses the dependency axis (a flag yoloAI calls) and leaves the freshness axis open; they are different problems that look alike.
    **The hard part is the value, not the mechanism.** Nobody knows which version first carried the flags `internal/agent/agent.go` uses, so any floor chosen today is invented. Setting it to today's latest is an over-constraint that forces rebuilds on users whose older CLI worked fine; setting it lower is unverifiable. The defensible policy is to introduce it *empty or at a known-good low water mark* and raise it only when a release knowingly adopts a newer flag — which also makes the raise the natural place to record why.
- **Pointer:** `runtime/docker/resources/Dockerfile` (the unpinned npm `RUN` and its rationale comment); `runtime/docker/build.go` (`buildBaseImage`, which passes no `--no-cache`); `internal/agent/agent.go` (the `Definition` flag surfaces). Sibling: [DF156](findings-resolved.md).
