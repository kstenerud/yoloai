...
- **Severity:** MEDIUM (apple + seatbelt are macOS-only, the lighter isolation tier; the probe path is broken-metadata-only)
- **Disposition:** PARKED — container backends fixed; apple/seatbelt code + the probe residual remain
