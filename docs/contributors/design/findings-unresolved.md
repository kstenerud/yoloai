...
- **Remedies, none obviously right — this is a rebuild-frequency policy question, which is why it is filed rather than fixed:**
  ...
  - **Declare a floor.** Pin a *minimum* rather than an exact version, which is compatible with the unpinned intent (`@^X.Y.Z`), and makes the Dockerfile itself the fence: changing the floor changes the Dockerfile, which changes `buildInputsChecksum`, which rebuilds the base through the machinery DF156 already built. Does nothing about drift *above* the floor.
