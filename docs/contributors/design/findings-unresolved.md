...
- **Trigger:** implement the daemon-posture gate (detect `docker info` `SecurityOptions` `name=userns`; refuse `:overlay` otherwise) and/or the host-side-overlay rethink; until then ship the v0.6.0 interim (treat `:overlay` as an explicit documented dangerous opt-in with a loud warning) — now the **recommended standing posture**, not just a stopgap.
