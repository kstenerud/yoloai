...
- **Severity:** MEDIUM
- **Disposition:** ESCALATED (security; fix needs empirical gVisor validation before it can land)
- **Description:** In `container-enhanced` (gVisor) isolation, `state.Perms` returns `SecretsDir: 0755` / `SecretsFile: 0644`, and `provision.CreateSecretsDir` stages the secrets via `os.MkdirTemp(stagingRoot, "yoloai-secrets-*")` where `stagingRoot` defaults to `""` → `os.TempDir()` (`/tmp` on Linux). `/tmp` is world-traversable, so for the window between staging and removal (the entrypoint reads `/run/secrets`, signals the consumed marker, then the host `RemoveAll`s the dir — up to `SecretsConsumedTimeout`, 30s default), **any local user on a multi-tenant Linux host can read the API keys / OAuth tokens** (`ANTHROPIC_API_KEY`, `CLAUDE_CODE_OAUTH_TOKEN`, `CODEX_API_KEY`, …). The nolint justification ("removed within seconds") describes the window but does not close the race. Standard Docker mode is unaffected (it uses `0700`/`0600`). The broad bits are a real gVisor constraint, not laziness: the gofer/pre-remap container UID must read files the host Go process created (`perms.go` comment), and `--userns=keep-id:uid=1001` maps the container to the host user.
- **Fix (designed, unvalidated):** default `Layout.SecretsStagingDir` to a **user-private `0700` parent** (e.g. `<DataDir>/run/`, created `0700` and chowned to the real user under sudo) instead of bare `/tmp`. The inner dir/files keep `0755`/`0644` for the gVisor reader, but a `0700` parent that other humans cannot traverse neutralizes the world bits. The plumbing already exists end-to-end (`CreateSecretsDir(stagingRoot)` ← `Layout.SecretsStagingDir`); only the default is unsafe. The strongest variant (Linux-only, deferred) is a tmpfs/`memfd`-backed staging root so plaintext never hits a persistent path.
- **Open question (blocks landing):** does the gVisor gofer / pre-remap container reader traverse a `0700` parent owned by the real host UID? If it reads **as the host user** (likely under `keep-id`), the `0700` parent works *and* the inner files could drop to `0600` (strictly better). If it reads **as a userns-mapped subuid**, a host-UID-owned `0700` parent blocks it and we need `0710`/`0711` + a shared group, or the tmpfs route. Requires a gVisor smoke test asserting (a) the agent authenticates (secrets readable) and (b) a second local user cannot read the staged files.
- **Pointer:** `internal/sandbox/state/perms.go:31-32`; `internal/sandbox/provision/provision.go:33`; staging root threaded from `Layout.SecretsStagingDir` via `internal/sandbox/launch/launch.go:48`.
