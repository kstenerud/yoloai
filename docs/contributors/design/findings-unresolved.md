...
- **Deliberately NOT given a smoke fingerprint.** The autopsy suggested one ("likely a new failure mode… add a fingerprint"), and it would be wrong here: a fingerprint matching `ttrpc: closed` would classify a future *deterministic* regression as this known flake, and DF72 is proof that this exact string carries serious causes. A one-occurrence non-reproducing failure does not have enough shape to suppress safely; suppression would cost more than the flake does.
...
### DF161 — mount conformance is skipped on the only two backends whose mounts are unusual, over one hardcoded path; and seatbelt's read-only is grant-omission, not a deny
...
- **A trap for whoever fixes (a).** The `ReadOnly` assertion is only meaningful when its fixture sits outside every broad grant, and on seatbelt the system temp dir is inside one — which is exactly where `t.TempDir()` puts it. Un-skipping without addressing that lands a test failing for a reason unrelated to what it asserts, which is worse than the skip. The fixture root has to come from somewhere the backend agrees is not broadly granted.
