...
- **Already live, not hypothetical.** The `host/` move landed in the branch's step 2, so `migrate_agentcfg.go:64` and `migrate_principal.go:286` are pointing at `host/environment.json` today, and `migrate_overlay.go` has four such sites plus a literal `if e.Name() == "work"` scan of the sandbox root that a tiered root no longer satisfies.
...
- **Design settled 2026-07-31: [staged-migration.md](plans/staged-migration.md).** The whole ladder runs in scratch and promotes once, each step a shape → shape transform assuming its own era's layout, so no migrator needs to know about tiers. The stamp is the resume point (no journal, D121), preconditions like "sandboxes must be stopped" become a second axis on `Op` unioned over the planned set, and `repopulate` is kept at the tree level and disabled at the per-sandbox level.
