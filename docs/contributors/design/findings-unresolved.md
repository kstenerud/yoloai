...
- **Disposition:** PARKED — needs a dedicated design pass, not just implementation (user: "definitely going to need some more thinking to get right").
...
- **Disposition:** PARKED (user: "a concern for later").
...
- **Disposition:** OPEN (S3.3 session/completion-layer work). **Pre-existing, not a carve regression** — but the carve makes in-place relaunch the *common, reliable* path (the box always stays up), so the gap now bites where it used to be rare.
