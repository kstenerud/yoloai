...
### DF111 — the tart base VM's git needs an Xcode licence agreement, so in-VM work-copy git fails exit 69

- **Discovered:** 2026-07-15 · **Workstream:** releasetest on Apple Silicon (post-DF94 rerun)
- **Severity:** MEDIUM (blocks `TestIntegrationTart_GitCorruption`; any in-VM git on a fresh `yoloai-base` VM is exposed)
- **Disposition:** PARKED — needs the Mac; filed, not fixed
- **Description:** `TestIntegrationTart_GitCorruption` fails with `exec exited with code 69: You have not agreed to the Xcode license agreements. Please run 'sudo xcodebuild -license'`. The VM's `git` is Apple's `/usr/bin/git` shim, which refuses to run until the licence is accepted. This matters beyond the test: D113 confines copy-mode work-copy git **inside** the VM for tart, so every in-VM git call sits behind this. Note the sibling tests (`MultipleAuxDirs`, `VMLocalStorageVerification`) passed, so it only bites where git actually executes — which is why it survived to now.
- **Open questions for whoever picks this up:** is the licence acceptance missing from the `yoloai-base` VM provisioning (a base-image gap, fix once at build), or is it host state that the VM inherits? Does `git --version` alone trigger it, or only real subcommands? Accepting a licence non-interactively in a provisioning step needs care — `sudo xcodebuild -license accept` requires root in the VM.
- **Pointer:** `internal/orchestrator/integration_tart_test.go:377`; tart base-image provisioning; D113 (in-VM git confinement).

### DF110 — test principals are unique per PROCESS, so a crashed run's leaked VM is adopted by the next run as "already running"
...
- **Disposition:** PARKED — the mechanism is verified by reading; the fix needs the Mac to confirm
...
- **The failure chain, all four links confirmed against the 2026-07-15 log:** (1) an earlier run died by timeout — DF94 records `panic: test timed out` with a `wait4` stack — and a panic skips `t.Cleanup`, so the VM leaked despite `t.Cleanup(destroySandbox)` at `integration_tart_test.go:114`; (2) the next run recomputed the same instance name; (3) `DetectStatus` found that VM running → `StatusActive` → `lifecycle.Start` took the "already running" no-op, which is why the log shows `starting sandbox` with **no** `recreating container`/`launching container` line and the test took 1.77s against 43–50s for its passing siblings; (4) work-dir setup therefore never ran, so `BaselineSHA` was empty (`:137`) and the in-VM `git -C` hit a path the *leaked* VM never had — the failure text names `TestIntegrationTart_FullLifecycle2070812500`, a per-run temp suffix, which is the proof it was provisioned by a different run.
- **The design tension to settle, not paper over.** The obvious fix — mix the PID or a random seed into the principal — makes each run unique but makes leaked VMs **unreapable**: prune is principal-scoped, so debris under a random principal is orphaned forever with a name nobody can reconstruct. The deterministic `t0000001` is what makes leftovers findable. So the choice is roughly: (a) keep it deterministic and make the tier refuse to adopt a pre-existing instance (destroy-then-create, or fail loudly naming the leftover — turns a silent adoption into a clear error), (b) sweep the `yoloai-t*` namespace before the tier runs (idempotent, but hostile to concurrent test processes), or (c) go unique and add a reaper that can find debris without knowing the principal. (a) looks smallest and closes the false-PASS risk directly.
- **Immediate unblock:** `tart list` then delete any `yoloai-t*` VM; re-run the tier.
- **Pointer:** `internal/testutil/isolation.go:26` (`UniqueTestPrincipal`), `store/paths.go:135` (`InstanceName`), `internal/config/names.go:82` (`InstancePrefix`), `internal/orchestrator/lifecycle/start.go` (the StatusActive no-op), `internal/orchestrator/integration_tart_test.go:114,137,158`. Related: DF94 (the timeouts that leaked the VMs), DF19.
