...
- **Disposition:** FIX IN PROGRESS (fix **B** — a first-class `ProcSpec.Detached` on the substrate Launch primitive; docker uses `ContainerExecStart{Detach:true}` + stdio redirected to `/yoloai/logs/session-runner.log`)
