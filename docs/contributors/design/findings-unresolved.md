...
- **Docker is immune, and the reason indicts the other two.** It filters by label **equality** — `runtime.IsOrphanCandidate`, `labels[LabelPrincipal] == string(principal)` (`runtime/orphan.go:26-30`) — so `""` matches only `""`. That is exactly what D62 specified: "Labels carry identity for fail-closed enumeration… runtime enumeration filters by **label, not by name-string splitting** (Tart has no native label → store the map in the sandbox dir)." Tart and seatbelt implement the name-string half D62 explicitly ruled out; containerd is mixed (`prune.go:109,133` uses `InstancePrefix` + `HasPrefix` alongside one label reference) and needs the same audit.
...
- **Verified by reading, not run.** The chain is four short predicates and a fact about strings (`"yoloai-acme-foo"` starts with `"yoloai-"`); each link was read at the cited lines. Not executed end-to-end: it would need a second principal's instances on this host, which means standing up an integrator against the same backend — worth doing when (a) or (b) is implemented, as the regression test for whichever lands. Naming the check rather than claiming the run, per this file's rule.
