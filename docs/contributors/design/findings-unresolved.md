...
**Append a new entry at the end of `## Findings`, below the last one.** Allocation is inherently
check-then-use — the script tells you 155, you write it up, and another PR can take 155 in the
meantime, which is exactly what happened when PR #44 allocated DF153 on its own branch. Nothing
closes that window. Appending narrows the consequences instead: two PRs that both allocate the
same number then insert at the same place in this file, so the collision surfaces as a **merge
conflict** at rebase rather than as a surprise later. The real guarantee is downstream of both —
`TestRepoHygiene_DecisionCitations_ResolveAndAreUnique` fails `make check` on any duplicate DF
heading, so a collision cannot ship regardless. (The entries above the append point are in no
particular order for historical reasons; they are not worth re-sorting, and new ones going to the
end is what makes the conflict happen.)
...
- **Disposition:** ADDRESSED-IN-PLACE (D128, then D129 after a second round of evidence); the mechanized gate is PARKED
...
- **If this is revisited**, the defensible narrow form is a gate on `yoloai-<profile>` alone — profile image tags always go through `config.ProfileImageTag`, so that single form is provably scoped. It currently appears zero times outside exempt history, so it would guard a form that has already stopped occurring. Prefer instead to keep name drift where D126's own remediation put it: a rule-2 sweep at rename time, review-caught, exactly as D122 left D/DF citations.
...
- **Sequenced in** [`plans/profile-image-lifecycle.md`](plans/profile-image-lifecycle.md) — independent of the interface change and parallelisable. First check whether containerd's link step generalises to an arbitrary tag or is special-cased to `yoloai-base`; if the latter, that is the actual work.
...
### DF154 — nothing verifies the profile image exists, and a pre-flight check cannot be the fix
...
- **Disposition:** PARTIALLY ADDRESSED 2026-07-29 — the diagnostic half is built (`missingImageHint`, `internal/orchestrator/launch/launch.go`, wired into both `rt.Create` sites and covered by `TestMissingImageHint`, verified to fail on revert). A failed launch now names the image, says the build record tracks the Dockerfile's checksum rather than the image's existence, and gives the rebuild command. **Still PARKED: automatic recovery** — rebuilding and retrying rather than telling the user to. That needs the profile name, the secrets and the build path at the launch seam, which currently has the sandbox state but not the build inputs, so it is a real plumbing change rather than a wrap.
...
- **Sequenced in** [`plans/profile-image-lifecycle.md`](plans/profile-image-lifecycle.md) — the remaining recovery half leads the sequence, because it is what lets every upstream staleness check be an optimisation that is allowed to be wrong.
