...
| [agent-layer.md](agent-layer.md) | **DESIGN IN PROGRESS** (no D-number). Per-agent adapters over the agent-free lower layers — gathers today's scattered agent-aware logic into one layer. Core principle: an agent declares the *mechanism*, the consuming layer supplies the *payload* (agent owns no cross-layer payload); capability groups each owned by one consuming layer; declaration = data + a thin code-adapter seam; registry closed-but-seam-shaped. Has a RESUME-HERE (re-homing map, public surface). |
