...
| W5 | — |
| W10 | `w10-default-network-gateway.txt`. **`reach.go`'s central premise does not hold on this host, and it is a shipped path.** Every yoloAI apple sandbox lands on the built-in `default` network (`runtime/apple/apple.go:236` never passes `--network`), and `InjectorReach` binds the credential injector on that network's gateway and has the guest dial it there — stated in the file as *"Verified on the 2026-06-28 Mac spike"*. Measured now: a guest on `default` **cannot reach a host listener on its gateway** (`000`) and **cannot resolve through it** either, while holding full external egress (`301` to 1.1.1.1) — and a guest on a *created* per-sandbox network reaches its gateway on the same port in the same second (`204`, DNS ok). Two trials agree; a third was **voided** because DF190 displaced one guest's network entirely, which is that finding reproducing between a created network and the built-in one. No pf rule is loaded at any point. `ipAssignedToHost(gw)` is true, so `InjectorReach` does *not* degrade — it succeeds and the injector binds an address the agent cannot dial. **The mechanism is not established** and neither is whether it was ever true; the `container` CLI here is 1.0.0 against a June spike. |
