...
| V6 | Does a counter-vs-`rx_packets` detector report **UNKNOWN** for an idle sandbox rather than healthy or broken? | Part 2's exit criterion, scoped to the faults V1 **and V1b** leave uncovered. The macOS canary shipped this exact free negative once. **May be dropped** if V1b covers the remainder. | low |
