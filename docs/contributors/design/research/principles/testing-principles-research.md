> **ABOUTME:** Primary-source evidence backing each principle in testing-principles.md — each
...
`docs/contributors/standards/go.md` §Error Handling) creates seams that make error paths
...
include error inputs by default — `docs/contributors/standards/go.md` §Testing).
...
Applied to yoloAI: `docs/contributors/standards/go.md` §Testing states explicitly: "Bug
...
yoloAI follows this with `testify/assert` for assertion sugar (`docs/contributors/standards/go.md` §Testing: "testing stdlib + testify/assert — reduces assertion boilerplate") but no mock framework and no external fixture library.
...
yoloAI's coding standard (`docs/contributors/standards/go.md` §Testing): "Mocking:
