...
| `df181-timing.txt` (post-fix runs) | the acceptance check for DF181's fix — 16 consecutive replacements, zero `STALE`, against 2-in-5 before. A fake cannot show a wait outlasts a real revalidation tick, so this is the only evidence that can |
