...
| `agent-privilege-reality.txt` | **post-round, and it voids a recommendation this directory's own earlier runs produced.** Several runs here measured a *root* agent in a container the harness launched with a bare `--entrypoint /bin/sh`, and "run the agent non-root" was carried to the owner as the fix for the sysctl class. The product does not launch it that way: `entrypoint.py:324` execs `gosu yoloai …` branching on `running_as_root` rather than on backend, so the agent has been **uid 1001 on every backend all along** — there was nothing to implement. What was never checked is `Dockerfile:229`, `yoloai ALL=(ALL) NOPASSWD:ALL`. So the uid is not the variable and this run makes the **sudo grant** the mechanism, launching the agent exactly as the product does. Grant removed: sysctl / tamper / netlink all **refused**. Grant present, same uid, same launch shape: **`ok / ok / refused`** — the agent turns IPv6 back on and rewrites `firewall.py`, while netlink alone stays closed because the bounding-set drop is inherited across `sudo`. **Consequence: in-guest enforcement cannot be made tamper-resistant while the sandbox grants the agent root, which it does deliberately.** The baseline arm shows removing the grant closes every vector, and that is measured as a trade rather than proposed as a fix |
| `tamper-persistence.txt` | **post-round, and it answers "if we just overwrite our own files, is tampering still a problem?" with no.** Three results on one container across four restarts. (1) A tampered start script leaves the sandbox **unenforced** at the next start — `301` where the untampered baseline gives `000`, so *writable and persistent* becomes *and unenforced*, which is the step nobody had taken. (2) **With the start script byte-identical to the clean one** — the overwrite policy, applied, and controlled for — shadowing `iptables` with a no-op wrapper *also* leaves it unenforced. **So overwriting our own files is not sufficient; the residue is the toolchain.** (3) And running the agent **non-root does not protect our own files either**: it cannot append to a root-owned start script, but it can `unlink` and recreate it, because `Dockerfile:276` does `chown -R yoloai:yoloai /yoloai` so the **directory** belongs to the agent's own user — and `entrypoint.py:117` chowns the files themselves at runtime, removing even that. Reporting the append refusal alone would have said the non-root agent was contained |
| `proc-sys-net-census.txt` | **post-round. The census `agent-uid-and-sysctls.txt` said was owed, and the answer is a class not an instance.** A root agent with `CAP_NET_ADMIN` dropped can write **763 of the 780** files under `/proc/sys/net` — including 10 of the 12 knobs judged containment-relevant (`disable_ipv6`, `accept_ra`, `rp_filter`, `accept_local`, `route_localnet`, `ping_group_range`, `send_redirects`, `arp_ignore`, `arp_announce`). The **non-root** agent, same bounding-set drop, can write **0 of 779**. Every write is idempotent — the current value read and written back — so the census cannot change behaviour while measuring it, and `flush` triggers and write-only files are skipped. `ipset` is refused in the same arm, which is what proves the drop took and makes the sysctl successes a statement about `/proc/sys` rather than about a drop that silently failed. **Bounded:** the relevant/irrelevant split is a *judgement* about what each knob does; only `disable_ipv6` is demonstrated end-to-end. Run 1 voided on a population control (780 vs 779) that was right to fire — five files are mode 0600 and root-readable only, so the difference is now named and bounded rather than asserted away |
| `agent-uid-and-sysctls.txt` | **post-round, and it bounds the bounding-set drop.** Everything `w6` and `cap-drop-coverage` measured as refused goes through **netlink**, which genuinely checks `CAP_NET_ADMIN`. **`/proc/sys/net` does not.** `disable_ipv6` is `-rw-r--r-- root root`; the kernel widens those bits for a `CAP_NET_ADMIN` holder and otherwise returns the file's own mode, which grants **write to its owner** — and the agent is uid 0. So a root agent with the capability dropped re-enables IPv6 with one `echo`, and although `ip -6 addr add` and `ip -6 route add` stay refused, **the kernel reconfigures the address and default route itself** from the RA. Dropping `CAP_DAC_OVERRIDE` as well was tried and **does not** close it — kept in the file, because the refuted mechanism is what pointed at ownership. What closes it is the agent **not being root** (`refused / 1 / 0` against root's `ok / 0 / 1`), which is exactly what the docker path already does with `gosu` and the apple path does not (DF179). The two compose: non-root closes the sysctl path, the bounding-set drop still holds if root is regained |
| `ipv6-allowlist-remedy.txt` | **post-round. The chosen remedy for the v6 hole, measured end to end rather than proposed.** An `inet6` ipset plus `ip6tables -m set --match-set` works on the blessed image — `xt_set` for the v6 family is present, which `firewall.py`'s own v4 fallback shows is not guaranteed. It **denies** (peer not in the set: `000`, against a `200` baseline taken on the same rig) and it **permits** (adding the peer's address: `200` again), which is what separates an allowlist from an outage. And it **does not take the v4 half down**: the v4 chain still permits its own allowlisted peer throughout. **DF134's hazard is confirmed still live** — `iptables -d <v6 addr>` fails with *host/network not found* — but bounded: the v4 chain is byte-identical either side (3 rules → 3 rules), so the danger is a caller aborting an install part-way, not iptables corrupting itself. The families must be populated independently. **Untried, and it is the real cost of this option: AAAA resolution**, which doubles the divergence and decay surface already measured for v4; also ICMPv6/NDP under a v6 default-deny, and link-local traffic |
