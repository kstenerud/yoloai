...
- **Opting in to credential access:** Users who need SSH agent forwarding or other credentials can add them via the config `env:` section (e.g., `SSH_AUTH_SOCK`) which flows through the secrets mechanism, or via `directories:` for directory access (e.g., `{path: ~/.ssh, mode: ro}`). `directories:` replaced `mounts:` in [D142](../decisions/working-notes.md) and additionally carries the aux-dir guards `mounts:` had none of: dangerous-path refusal, path-overlap and duplicate-mount detection, and the dirty-repo gate. Note the dangerous-path list is `$HOME`, `/`, and system directories (`internal/workspace/safety.go:12-24`) — **`~/.ssh` is not on it**, so mounting it is still permitted without `:force`. The advice above stands on its own: forward the agent socket rather than the key material.
