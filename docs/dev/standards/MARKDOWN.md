...
Every yoloAI source file — Go, Python, shell, or Markdown under `docs/dev/` — opens with an ABOUTME comment block. The convention is from `~/.claude/CLAUDE.md`:
...
- All Markdown source documents under `docs/dev/` (principles, standards, research, plans, working-notes, ARCHITECTURE, etc.).
