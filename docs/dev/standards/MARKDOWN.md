...
Every yoloAI source file — Markdown included — opens with an ABOUTME comment block when it lives under `docs/dev/`, `runtime/`, `internal/`, `cmd/`, or any directory where source files live. The convention is from `~/.claude/CLAUDE.md` and is enforced by reading:

```markdown
ABOUTME: One-line description of what this doc is for.
ABOUTME: Continue here if needed; keep under 80 chars each.
```

The exception: user-facing docs in `docs/` (the GUIDE, BREAKING-CHANGES, ROADMAP, README) do not carry ABOUTME headers — they are content destination, not source context. The split: developer-facing context-carrying docs get headers; user-facing reference docs do not.

When in doubt: if the doc is in `docs/dev/` or alongside code, ABOUTME header. If it's in `docs/` (top level inside docs/, like `docs/GUIDE.md`), no header.
...
- **Relative paths from the current file.** A doc at `docs/dev/principles/foo.md` linking to `docs/dev/working-notes.md` uses `../working-notes.md`, not the absolute path. Relative links survive when files move.
- **For files outside the current subtree**, use the path relative to repo root for clarity (e.g., from `docs/dev/research/principles/x.md` referencing `internal/runtime/registry/descriptor.go`, write `internal/runtime/registry/descriptor.go`, not `../../../../internal/runtime/registry/descriptor.go`).
...
- **All-caps for legacy / formal docs in `docs/dev/`**: `ARCHITECTURE.md`, `OVERVIEW.md`, `RESEARCH.md`, `CRITIQUE.md`, `OPEN_QUESTIONS.md`, `SENTIMENT.md`. These predate the principles/standards split.
...
- **Don't mix two file-naming conventions in one directory**. New `docs/design/` files are kebab-case; new `docs/dev/` standards / principles files follow the convention they live next to.
