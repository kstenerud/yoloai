...
This standard covers the base image. Profile Dockerfiles are user-owned; yoloAI documents the `yoloai-base` contract in `docs/design/config.md` and trusts users to apt-install what they need on top.
...
- **Rejected alternatives**: Alpine (musl libc → glibc-specific tools fail; the Bun-bundled Claude Code installer had documented issues per `docs/dev/OPEN_QUESTIONS.md` #2), Ubuntu (no advantage over Debian; larger), distroless (can't apt install at runtime, doesn't compose with profile system).
...
- `nodejs` — pinned via NodeSource repository to Node 22 LTS (rationale: `docs/dev/OPEN_QUESTIONS.md` #2).
...
Profile Dockerfiles are NOT hadolint-checked by yoloAI's CI — they're user-authored. The hadolint discipline is documented as a recommendation in `docs/design/config.md`.
...
This is documented in `docs/dev/ARCHITECTURE.md`; the Dockerfile only needs to provide the *environment* the entrypoints will run in.
...
- `docs/design/config.md` — profile system design (how user profiles compose with the base image).
