...
100. **Dual command dispatch (verb-first vs name-first)** — yoloAI supports both `yoloai diff <name>` (verb-first) and `yoloai sandbox <name> diff` (name-first), plus aliases (`ls`, `log`, `exec`) and `YOLOAI_SANDBOX` env-var resolution. Implemented via custom argument parsing in `internal/cli/commands.go` and `envname.go`; Cobra doesn't do this natively. Every command author must think about both dispatch paths and the help text must make both sensible — the test surface effectively doubles. Question: are users using both forms, or has one form dominated such that the other can be deprecated? Before deciding, need usage data (project has no telemetry today). If both well-used, keep them and add a test that asserts every command works through both paths. If one dominates, deprecate the other with a quiet warning, then remove in a future breaking-changes window. Tracked here rather than in [`plans/architecture-remediation.md`](plans/architecture-remediation.md) because it's a decision that needs data, not a refactor.

101. **`yoloai.Client` public API fate** — `yoloai.go` exports a high-level Go API (`Client`, `Run`, `Diff`, `Apply`, `List`, `Inspect`, `Stop`, `Destroy`) intended for library consumers. ARCHITECTURE.md calls it "the public Go API for library consumers." Question: does any external consumer actually use it? If yes, document them in CLAUDE.md and declare API stability in BREAKING-CHANGES.md; the constraint constrains everything downstream (e.g., W7 of architecture-remediation.md can't move config typed errors without re-exporting). If no, collapse `yoloai.go` into `internal/` and stop maintaining an API-stability surface with no consumers. Tracked here rather than in [`plans/architecture-remediation.md`](plans/architecture-remediation.md) because it's a decision needing data (do external consumers exist?), not a refactor.

102. **`sandbox/setup_test.go` is 632 lines — justified or split?** The audit flagged this test file as longer than typical and asked whether the unit under test (`sandbox/setup.go`'s interactive first-run flow) is doing too much, or whether the test length is genuinely earned by exhaustive coverage of an interactive flow. Decision: read `setup.go` + `setup_test.go` together; if the test reads as exhaustive table-driven coverage of legitimately many branches, leave it and add a top-of-file comment justifying the length. If the test reads as repetitive setup boilerplate or as covering for an over-large unit, split (likely along sub-flow boundaries — tmux config selection, default backend selection, default agent selection). Tracked here rather than in [`plans/architecture-remediation.md`](plans/architecture-remediation.md) because the work is decision-then-maybe-refactor, not a refactor with a known shape.
