...
96. **Profile env var / agent_args unset mechanism** — Env vars set to empty string (`MY_VAR: ""`) remain defined in the container, which differs from being absent — scripts using `${MY_VAR+x}` to check for existence will still see the variable. A child profile cannot remove an inherited env var or agent_arg. A sentinel value (e.g. `!unset`) could allow child profiles to explicitly remove inherited keys. The `--diff` output would show these as removals. Low priority unless users report inheritance conflicts.
