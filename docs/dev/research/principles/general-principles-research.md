...
system paths (`docs/design/security.md`); `:copy` mode default for workdir
...
letting the sandbox run indefinitely. The disk pre-flight (`docs/dev/
...
- **Dangerous-directory detection:** `docs/design/security.md` §Security
  Considerations — refuses `$HOME`, `/`, macOS system directories, Linux system
  directories unless `:force` is appended.
- **Dirty-repo warning:** `docs/design/security.md` — warns when `:rw` is used
...
config.yaml per profile, `docs/design/README.md` §Architecture) means users get
...
(`docs/design/security.md` §Credential Management), and the container runs as user
...
- **Name required:** The sandbox name is a required positional argument; no
  ...
  required (no auto-generation)" (`docs/design/README.md` §Design Principles). The
  rationale: auto-generated names lead to sandbox sprawl and make `yoloai apply`
  ambiguous.
- **Dirty-repo warning:** `docs/design/security.md` — "`:rw` directories give the
...
Internal source: `docs/dev/working-notes.md`. The D-numbered entries (D1 onward)
...
MIT-licensed but less well-documented). Public design docs (`docs/design/`),
public idiosyncrasies catalog (`docs/dev/backend-idiosyncrasies.md`), public
BREAKING-CHANGES (`docs/BREAKING-CHANGES.md`), public roadmap (`docs/ROADMAP.md`),
and public security decisions (`docs/design/security.md`) are competitive
...
security doc (`docs/design/security.md`) and the backend-idiosyncrasies catalog
(`docs/dev/backend-idiosyncrasies.md`) are the implementation. SolarWinds: US-CERT
...
context before the operation begins. The `docs/dev/working-notes.md` memory file
...
(`docs/design/README.md` §Value Proposition) documents a concrete tradeoff:
...
capabilities. The `docs/design/security.md` documents this explicitly for each
...
`docs/design/security.md`). This is a concrete example of the cross-platform
...
handling. `docs/design/README.md` §Prerequisites: "Windows/WSL: Expected to work
...
### `docs/dev/backend-idiosyncrasies.md` as platform-verification record

Internal source: `docs/dev/backend-idiosyncrasies.md`. This document is the
...
Podman, Tart, Seatbelt), read `docs/dev/backend-idiosyncrasies.md`." This is
...
addressed in `docs/design/security.md`, but not a data-controller concern.
