...
not in scope. `docs/design/network-isolation.md` §Threat Model states this
...
Applied to yoloAI: `docs/design/security.md` §Security Considerations states this
...
stated in `docs/design/README.md` §Design Principles: "Defense-in-depth as opt-in
...
runs containers under gVisor's `runsc` runtime. `docs/design/security.md`
...
`docs/design/security.md` §Seatbelt Backend Security documents the default-deny
...
paths via Mach services in macOS Seatbelt. The `docs/dev/research/sandboxing.md`
...
- `:overlay` mode — `CAP_SYS_ADMIN` required for overlayfs mounts inside the
  container. `docs/design/security.md` states: "It is a broad capability — it also
...
- `--network-isolated` (host-side approach, per `docs/design/network-isolation.md`)
...
isolation design (`docs/design/network-isolation.md`) eliminates `CAP_NET_ADMIN`
...
and explicitly described as "a broad capability" in `docs/design/security.md`.
...
applying. `docs/design/security.md` §Security Considerations: "`:copy` directories
...
Applied to yoloAI: `docs/GUIDE.md` and `docs/design/commands.md` both describe
the three-step workflow as the core use case. `docs/design/README.md` §Design
...
Applied to yoloAI: the file-based injection flow at `docs/design/security.md`
...
Applied to yoloAI: `docs/dev/research/security.md` §1 "Risks of the Environment
...
Applied to yoloAI: `docs/dev/research/security.md` §3 documents this as "widely
...
`docs/design/security.md` §Credential Management documents the accepted tradeoff
...
is documented in `docs/dev/research/security.md` §1 and is accepted as a residual
risk.
...
1. **Filesystem (Seatbelt):** `docs/design/security.md` §Seatbelt Backend Security
...
2. **Network (`--network-isolated`):** `docs/design/network-isolation.md` §Design
...
3. **Credential env vars (Seatbelt):** `docs/design/security.md` §Seatbelt Backend
...
Applied to yoloAI: `docs/design/README.md` §Design Principles: "Safe defaults:
...
Applied to yoloAI: `docs/design/security.md` §Security Considerations documents
...
Note: `docs/design/network-isolation.md` §Threat Model calls DNS tunneling "out of
...
Applied to yoloAI: `docs/dev/research/sandboxing.md` §OverlayFS documents these:
...
Applied to yoloAI: `docs/dev/research/sandboxing.md` §sandbox-exec documents
...
Applied to yoloAI: `docs/design/security.md` §Security Considerations: "Network
...
`docs/design/security.md` §Security Considerations documents the DNS exfiltration
...
`docs/design/network-isolation.md` §Threat Model scopes this out deliberately:
...
`docs/design/security.md` §macOS + gVisor Compatibility: "gVisor is blocked on
...
`docs/design/security.md` §Security Considerations: "`:copy` mode avoids this
...
`docs/design/security.md` §Credential Management: "Accepted tradeoff: The agent
...
`docs/dev/research/security.md` §1 "What it does NOT protect against" lists the
...
`docs/dev/research/sandboxing.md` §sandbox-exec: "Security strength: Meaningful
...
setup, GPU passthrough). `docs/design/security.md` §Security Considerations:
...
- **Dangerous-directory detection:** `docs/design/security.md` — refuses to mount
...
- **`:rw` is explicit:** D4 in `docs/dev/working-notes.md` — "Implicit upgrade
