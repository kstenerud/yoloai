...
After W11 steps 1–3 and W12, the only structural work still owing is the final descriptor-aware registry shape (W11 step 4: `runtime.Register(name, factory, descriptor)` so callers can enumerate static facts without instantiating). W1b (remove the agent-wrap gate one release after W1a) remains calendar-bound. Targeted test-file splits flagged in W12's audit are deferred. Net of these residuals, both of the audit's material risks ("uncovered Python boundary" and "Runtime interface accreted past comfortable") are closed: the Python boundary is under test + version-gated, and the Runtime interface dropped from 24 to 18 methods with five optional adapters.
