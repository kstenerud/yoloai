...
| F2 — 24-method `Runtime` interface | **open** | W11 not yet started. `runtime-interface-shape.sh` confirms method count still 24, optional adapters still 3 (`UsernsProvider`, `WorkDirSetup`, `StdioExecer`). Catalog spike complete: see [`research/runtime-interface-spike.md`](research/runtime-interface-spike.md) — descriptor refactor is feasible (0% of `BackendCaps` values dynamic; abort threshold not hit). |
